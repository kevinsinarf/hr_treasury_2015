...
$SALARY_AVG = str_replace(',','',$_POST['SALARY_AVG']);
$SALARY_SEVENTY = str_replace(',','',$_POST['SALARY_SEVENTY']);
$SALARY_TRUST = str_replace(',','',$_POST['SALARY_TRUST']);
$PENSION_MONTHLY = str_replace(',','',$_POST['PENSION_MONTHLY']);
//END ใช้ร่วมกัน

//DIV 3
$ARR_SALHIS_ID = $_POST['SALHIS_ID'];
$ARR_UPS_SDATE = $_POST['UPS_SDATE'];
$ARR_UPS_EDATE = $_POST['UPS_EDATE'];
$ARR_UPS_SALARY = $_POST['UPS_SALARY'];
$ARR_UPS_MONTH = $_POST['UPS_MONTH'];
//END DIV 3
...
switch($proc){
		...
		try{
			...
			$rec_per = $db->db_fetch_array($query_per);
				...
				$fields = array(
					...
					'PENSION_TYPE_GPF' => $PENSION_TYPE_GPF,
			...
			if(count($ARR_SALHIS_ID) > 0){
				foreach($ARR_SALHIS_ID as $index => $val){
					$fields = array(
						'SALHIS_ID' => $val,
						'PENSION_ID' => $PENSION_ID,
						'UPS_SDATE' => conv_date_db($ARR_UPS_SDATE[$val]),
						'UPD_EDATE' => conv_date_db($ARR_UPS_EDATE[$val]),
						'UPS_SALARY' => str_replace(',','',$ARR_UPS_SALARY[$val]),
						'UPS_MONTH' =>  str_replace(',','',$ARR_UPS_MONTH[$val]),
						'DELETE_FLAG' => 0,
						'CREATE_BY' => $USER_BY,
						'CREATE_DATE' => $TIMESTAMP
					);
	...
	case "edit" : 
		try{
			...
			$db->db_delete($TB3," PENSION_ID = '".$PENSION_ID."' ");
			if(count($ARR_SALHIS_ID) > 0){
				foreach($ARR_SALHIS_ID as $index => $val){
					$fields = array(
						'SALHIS_ID' => $val,
