...
if($S_PENSION_IDCARD != ''){
	$query = $db->query("SELECT TOP 1  * FROM PENSION_MAIN A 
								...
								WHERE A.PENSION_IDCARD = '".str_replace('-','',$S_PENSION_IDCARD)."' AND A.DELETE_FLAG = 0 AND A.POSTYPE_ID = 5 ORDER BY PENSION_TIME DESC ");
	...
	if($num > 0){
		...
		$QueryMain = $db->query("SELECT * FROM  PER_PROFILE A JOIN SETUP_POSITION_TYPE B ON A.POSTYPE_ID = B.POSTYPE_ID  
											WHERE PER_IDCARD =  '".str_replace('-','',$S_PENSION_IDCARD)."' AND A.DELETE_FLAG = 0 AND ACTIVE_STATUS = 1 AND A.POSTYPE_ID = 5 ");
		$rec_main  = $db->db_fetch_array($QueryMain);	
...
$arr_pension_type = array(1 => 'บำเหน็จ', 2 => 'บำเหน็จรายเดือน', 3 => 'บำเหน็จตกทอด');
...
<script src="js/pension_record_emp_form.js?<?php echo rand(); ?>"></script>
</head>
...
<div class="container-full">
  ...
  <div class="col-xs-12 col-sm-12">
    <ol class="breadcrumb">
      ...
      <li><a href="pension_record_emp_disp.php?<?php echo url2code($link2); ?>">บันทึกข้อมูลการขอบำเหน็จบำนาญ (ลูกจ้างประจำ)</a></li>
      ...
      <form id="frm-input" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
       ...
       <input type="hidden" id="PER_ID" name="PER_ID"  value="<?php echo $PER_ID; ?>">
            ...
            <div id="collapse9" class="collapse">
                 <?php
					...
					$total_all_cal = 0;
				 	...
				 	if($total_year > 0){
						$total_year_cal = ($total_year*12);
					}
				    if($total_month > 0){
						$total_month_cal = $total_month;
					}
					if($total_day > 0){
						$total_day_cal = ($total_day/30);
					}
				   $total_all_cal = ($total_year_cal + $total_month_cal) + $total_day_cal;
				   
				   $PENSION_SALARY = $PER_SALARY;
				   $PENSION_ALL = ($PENSION_SALARY * $total_all_cal)/12;
				 ?>
            	 <div class="row formSep">
            ...
            <div class="row head-form">
                <div class="col-xs-12 col-md-2" style="white-space:nowrap;">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapse10" onClick="$('.switchPic10').toggle();">
                        ...
                        บำเหน็จรายเดือน/บำเหน็จตกทอด
                    </a>
                </div>
            </div>
            <div id="collapse10" class="collapse">
               <?php 
			        $PENSION_MONTHLY = ($PENSION_SALARY*$total_all_cal)/600;
			   		$LEGENCY_ALL = ($PENSION_MONTHLY*15);
					
			   ?>
            	<div class="row formSep">
                    <div class="col-xs-12 col-md-2" style="white-space:nowrap;">เงินเดือน เดือนสุดท้าย : </div>
                    <div class="col-xs-12 col-md-2">
                    	<input type="text" id="PENSION_SALARY" name="PENSION_SALARY" class="form-control " value="<?php echo number_format($PENSION_SALARY,2); ?>" onBlur="NumberFormat(this,2);" >
					</div>
                    <div class="col-xs-12 col-md-1"></div>
                    <div class="col-xs-12 col-md-2" style="white-space:nowrap;">เงินบำเหน็จรายเดือนที่ได้รับ : </div>
                    <div class="col-xs-12 col-md-2">
                    	<input type="text" id="PENSION_MONTHLY" name="PENSION_MONTHLY" class="form-control " value="<?php echo number_format($PENSION_MONTHLY,2); ?>" onBlur="NumberFormat(this,2);" >
                    </div>
                </div>	
        ...
        <div class="col-xs-12 col-sm-12" align="center">
          ...
          onClick="self.location.href='pension_record_emp_disp.php?<?php echo url2code($link2);?>';">ยกเลิก</button>
        </div>
        <div class="clearfix"></div>
        <br>
      </form>
