$(document).ready(function(){
	...
	if($('input:radio[name^=REWARD_ORG_TYPE]:checked').length > 0){
		var REWARD_ORG_TYPE = $('input:radio[name^=REWARD_ORG_TYPE]:checked').val();
		chkType(REWARD_ORG_TYPE);
	}else{
		$('#REWARD_ORG_TYPE_1').prop('checked',true);
		var REWARD_ORG_TYPE = $('input:radio[name^=REWARD_ORG_TYPE]:checked').val();
		chkType(REWARD_ORG_TYPE);
