var url_process = "process/profile_bonus_process.php";
...
function addData(){
	$("#proc").val("add");
	$("#frm-search").attr("action","profile_bonus_form.php").submit();
}
...
function getORG(obj){
	...
	$.post('process/profile_bonus_process.php', {'proc':'get_org', ORG_PARENT_ID:val}, function(data){
...
function getlevel(value,POSTYPE_ID){
	var url ='process/profile_bonus_process.php';
	var html = "<option value=''></option>";
	$.ajax({
		url: url,
		type: 'POST',
...
function getLineGroup(TYPE_ID,POSTYPE_ID){
		var html = "<option value=''></option>";
		$.post('process/profile_bonus_process.php',{proc:'getLineGroup',TYPE_ID:TYPE_ID,POSTYPE_ID:POSTYPE_ID},function(data){
			$.each(data,function(index,value){
					html += "<option value='"+value['ID']+"'>"+value['VALUE']+"</option>";	
			});	
			$('#LG_ID').html(html);
