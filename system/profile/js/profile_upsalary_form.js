var url_process = "process/profile_upsalary_process.php";
...
function chkinput(){
	...
	if($("#POS_YEAR").val() == ""){
		alert("ระบุ "+$('#POS_YEAR').attr('placeholder'));
		$("#POS_YEAR").focus();
		return false;
	}
	if($("#ORG_ID_1").val() == ""){
...
function getORG(obj){
	...
	$.post('process/profile_upsalary_process.php', {'proc':'get_org', ORG_PARENT_ID:val}, function(data){
		$.each(data,function(index,val){
			html += "<option value='"+val['ID']+"'>"+val['VALUE']+"</option>";
	   });
	   $('#'+id_new).html(html);
	   $('#'+id_new).trigger('liszt:updated');
	   $('.selectbox').chosen({ allow_single_deselect: true, no_results_text: "No results matched"});
...
function getlevel(value,POSTYPE_ID){
	var url ='process/profile_upsalary_process.php';
	var html = "<option value=''></option>";
	$.ajax({
		url: url,
		type: 'POST',
...
function getLineGroup(TYPE_ID,POSTYPE_ID){
		var html = "<option value=''></option>";
		$.post('process/profile_upsalary_process.php',{proc:'getLineGroup',TYPE_ID:TYPE_ID,POSTYPE_ID:POSTYPE_ID},function(data){
			$.each(data,function(index,value){
					html += "<option value='"+value['ID']+"'>"+value['VALUE']+"</option>";	
			});	
			$('#LG_ID').html(html);
