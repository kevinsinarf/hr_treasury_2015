...
if(!empty($s_name_th)  && ($s_name_th!='undefined')){
   $ConName.=" AND (a.SP_FIRSTNAME_TH LIKE '%".ctext(trim($s_name_th))."%' OR a.SP_MIDNAME_TH LIKE '%".ctext(trim($s_name_th))."%' OR a.SP_LASTNAME_TH LIKE '%".ctext(trim($s_name_th))."%') ";
}
...
<button type="button" class="btn btn-primary" onClick="search_pop('form_set_penalty_form3.php','show_display',$('#name_serch').val());">ค้นหา</button>
<button type="button" class="btn btn-primary" onClick="search_pop('form_set_penalty_form3.php','show_display',$('#name_serch').val());">เพิ่มข้อมูล</button>
</div>
...
<div class="table-responsive">
	<table class="table table-bordered table-striped table-hover table-condensed">
		...
		?>
			<tr>
				<td align="center">
                <input type="radio" id="chk<?php echo $i;?>" name="chk" value="<?php echo $rec['SP_ID'];?>" onClick="return_name('<?php echo $rec['SP_ID'];?>','<?php echo $name;?>');"></td>
