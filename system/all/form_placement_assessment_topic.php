...
<?
		$sql_lea = "SELECT A.POS_NO,A.PER_IDCARD,A.PER_ID,A.PREFIX_ID,A.PER_FIRSTNAME_TH,A.PER_MIDNAME_TH,A.PER_LASTNAME_TH,A.PER_ID,B.LINE_NAME_TH,C.LEVEL_NAME_TH,D.MANAGE_NAME_TH,E.MT_NAME_TH,F.LG_NAME_TH ,G.TYPE_NAME_TH,A.ORG_ID_3,A.ORG_ID_4 FROM PER_PROFILE  A
				LEFT JOIN  SETUP_POS_LINE  B ON A.LINE_ID = B.LINE_ID
				LEFT JOIN SETUP_POS_LEVEL C ON A.LEVEL_ID = C.LEVEL_ID
				LEFT JOIN SETUP_POS_MANAGE D ON A.MANAGE_ID = D.MANAGE_ID
				LEFT JOIN SETUP_POS_MANAGE_TYPE E ON A.MT_ID = E.MT_ID 
				LEFT JOIN SETUP_POS_TYPE G ON A.TYPE_ID = G.TYPE_ID
				LEFT JOIN SETUP_ORG H ON A.ORG_ID_3 = H.ORG_ID 
				LEFT JOIN SETUP_POS_LINE_GROUP F ON F.LG_ID = A.LG_ID WHERE ORG_ID_3='".$org_id_3."'
				";
				$lea_query = $db->query($sql_lea);
				$nums=$db->db_num_rows($lea_query);
?>
<div class="row">
...
<button type="button" class="btn btn-primary" onClick="search_pop('form_placement_assessment_topic.php','show_display',$('#name_serch').val());">ค้นหา</button>
</div>
</div>
