...
/*	if($("#PER_IDCARD").val() == ""){
	...
	if($('#MOVEMENT_TYPE').val() == ''){
		alert('ระบุ '+$('#MOVEMENT_TYPE').attr('placeholder'));
		$('#MOVEMENT_TYPE').focus();
		return false;
	}
	if($('#MOVEMENT_OTHER_TH').val() == ''){
		alert('ระบุ '+$('#MOVEMENT_OTHER_TH').attr('placeholder'));
		$('#MOVEMENT_OTHER_TH').focus();
		return false;
	}
...
function getProcess(id){
			...
			$.post('process/data_profile_history_process.php',{proc:'getProcess',MOVE_PROCESS_GROUP:id},function(data){
			$.each(data,function(index,value){
					html += "<option value='"+value['id']+"'>"+value['value']+"</option>";	
			});	
			$('#MOVE_PROCESS_ID').html(html);
			$('#MOVE_PROCESS_ID').trigger("liszt:updated");
			$('#MOVE_PROCESS_ID').chosen({ allow_single_deselect: true, no_results_text: "No results matched"});
	},'json');
}
function delData(id){
