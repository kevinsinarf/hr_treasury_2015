...
if($s_name != ""){
	$filter .= " AND  (A.CT_NAME_EN like '%".ctext($s_name)."%' OR A.CT_NAME_TH like '%".ctext($s_name)."%') ";
}
if($s_status != ''){
	$filter .= " AND A.ACTIVE_STATUS = '".$s_status."' ";
}


$field=" A.*";
$table="SETUP_COMMAND_TYPE A 
LEFT JOIN SETUP_LAW_TYPE B ON A.LT_ID = B.LT_ID ";
$pk_id=" A.CT_ID";
$wh=" A.DELETE_FLAG = 0 {$filter}";
$orderby="order by B.LT_SEQ  ASC";
...
<script src="js/data_law_type_disp.js?<?php echo rand(); ?>"></script>
</head>
    ...
    <div class="groupdata">
        ...
        <div class="row">
			...
			<div class="col-xs-12 col-sm-3">
    			<input type="text" id="s_name" name="s_name" class="form-control" placeholder="ชื่อเอกสารอ้างอิง" value="<?php echo $s_name; ?>">
            </div>
            ...
            <table width="397" class="table table-bordered table-striped table-hover table-condensed">
              <thead>
                <tr class="bgHead">
                  ...
                  <th width="25%"><div align="center"><strong>ชื่อเอกสารอ้างอิงภาษาไทย</strong> </div></th>
                  <th width="15%"><div align="center"><strong>ชื่อเอกสารอ้างอิงภาษาอังกฤษ</strong> </div></th>
				...
				if($nums > 0){
					...
					while($rec = $db->db_fetch_array($query)){
						$edit = "<a data-toggle=\"modal\" class=\"btn btn-default btn-xs\" data-backdrop=\"static\" href=\"javascript:void(0);\" onClick=\"editData('".$rec["CT_ID"]."');\">".$img_edit." แก้ไข</a> ";
						$delete = "<button type=\"button\" class=\"btn btn-default btn-xs\" onClick=\"delData('".$rec["CT_ID"]."');\">".$img_del." ลบ</a> ";
						?>
						<tr bgcolor="#FFFFFF">
						  <td align="center"><?php echo $i+$goto; ?>.</td>
						  <td align="left"><?php echo text($rec['CT_NAME_TH']);   ?></td>
						  <td align="left"><?php echo text($rec['CT_SHORTNAME_TH']);    ?></td>
