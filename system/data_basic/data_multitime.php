...
$filter = "";
if($s_name != ""){
	$filter .= " and (MULTIME_ID like '%".ctext($s_name)."%' OR MULTIME_NAME_TH like '%".ctext($s_name)."%') ";
}
if($s_status != ""){
	$filter .= " and ACTIVE_STATUS = '".ctext($s_status)."' ";
}

$field="MULTIME_ID,MULTIME_NAME_TH,MULTIME_NAME_EN,ACTIVE_STATUS";
$table="SETUP_MULTITIME";
$pk_id="MULTIME_ID";
$wh="DELETE_FLAG='0' {$filter}";
$orderby="order by ACTIVE_STATUS desc, MULTIME_ID ASC";
...
<div class="container-full">
  ...
  <div class="col-xs-12 col-sm-12" id="content">
    <div class="groupdata">
        ...
        <input type="hidden" id="MULTIME_ID" name="MULTIME_ID" value="">
		<div class="row">
			<div class="col-xs-12 col-sm-2" style="white-space:nowrap;">ชื่อช่วงเวลาทวีคูณ :</div>
			<div class="col-xs-12 col-sm-3"><input type="text" id="s_name" name="s_name" class="form-control" placeholder="ชื่อช่วงเวลาทวีคูณ" value="<?php echo $s_name; ?>"></div>
            ...
            <table class="table table-bordered table-striped table-hover table-condensed">
              <thead>
                <tr class="bgHead">
                  ...
                  <th width="30%"><div align="center"><strong>ชื่อช่วงเวลาทวีคูณ (<?php echo $arr_txt['th'];?>)</strong> </div></th>
                  <th width="30%"><div align="center"><strong>ชื่อช่วงเวลาทวีคูณ (<?php echo $arr_txt['en'];?>)</strong> </div></th>
				...
				if($nums > 0){
					...
					while($rec = $db->db_fetch_array($query)){
						...
						if(chkPermission($menu_sub_id, 'edit')=='1'){
							$edit = "<a data-toggle=\"modal\" class=\"btn btn-default btn-xs\" data-backdrop=\"static\" href=\"javascript:void(0);\" onClick=\"editData('".$rec["MULTIME_ID"]."');\">".$img_edit." แก้ไข</a> ";
						}
						if(chkPermission($menu_sub_id, 'delete')=='1'){
							$delete = "<button type=\"button\" class=\"btn btn-default btn-xs\" onClick=\"delData('".$rec["MULTIME_ID"]."');\">".$img_del." ลบ</a> ";
						}
				?>
                <tr bgcolor="#FFFFFF">
                  <td align="center"><?php echo $i+$goto; ?>.</td>
                  <td align="left"><?php echo text($rec["MULTIME_NAME_TH"]); ?></td>
                  <td align="left"><?php echo text($rec["MULTIME_NAME_EN"]); ?></td>
