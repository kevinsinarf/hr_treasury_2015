...
// สำนัก
$q_org3 = $db->query("SELECT ORG_ID, ORG_NAME_TH FROM SETUP_ORG WHERE ORG_PARENT_ID = '15' AND ACTIVE_STATUS = '1' AND DELETE_FLAG = '0' ORDER BY case when ORG_SEQ is null then 1 else 0 end, ORG_SEQ ASC");
while($r_org3 = $db->db_fetch_array($q_org3)){
	$arr_org3[$r_org3['ORG_ID']] = text($r_org3['ORG_NAME_TH']);
}


$q_edit = $db->query("SELECT  SCORE_TYPE,LV_SCORE_ID, SCORE_S, SCORE_E, PERCENT_SAL,PERCENT_SAL_E FROM SAL_SCORE 
WHERE  POSTYPE_ID= '1' AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND SCORE_STATUS = 2  
ORDER BY SCORE_ID ASC");
$query_num = $db->query("SELECT COUNT(SCORE_ID) AS NUM_SCORE FROM SAL_SCORE WHERE POSTYPE_ID = '1' AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND SCORE_STATUS = 2 AND CONFIRM_TYPE = 2 ");
...
<script src="js/set_level_score_mg_disp.js?<?php echo rand(); ?>"></script>
</head>
<body>
		...
		<form id="frm-search" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                ...
                <table align="center" class="table table-bordered table-striped table-hover table-condensed" id="tb_data">
                    <thead>
                        <tr class="bgHead">
                            ...
                            <th width="20%"><div align="center"><strong>ร้อยละ</strong></div></th>
					...
					while($r_edit = $db->db_fetch_array($q_edit)){
                        ...
                        <tr id="<?php echo $id_tb;?>" >
							...
							?>
							<select name="LV_SCORE_ID[]" id="LV_SCORE_ID_<?php echo $id_tb;?>" placeholder="เลือกระดับ" class="selectbox form-control" style="width:100px;">
								<option value=""></option>
								<?php
            ...
            <div class="formlast">
                <div class="col-xs-12 col-sm-12" align="center">
                    ...
                    <button type="button" class="btn btn-default" onClick="ConfirmMg(2);">อนุมัติเกณฑ์การประเมิน</button>
                </div>
            </div>   
          <?php } ?> 
		</form>
