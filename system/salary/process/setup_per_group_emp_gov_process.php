...
$url_back="../setup_per_group_emp_gov_disp.php";
$TB = "SAL_UP_SALARY";
...
switch($proc){	
	case 'AddAll' :
		try{
		    $db->query('BEGIN TRANSACTION');
			  ...
			  $query_per = $db->query("SELECT PER_ID, PREFIX_ID, PER_FIRSTNAME_TH,  PER_MIDNAME_TH, PER_LASTNAME_TH, POS_ID, POS_NO, ORG_ID_1, ORG_ID_2,  ORG_ID_3, ORG_ID_4, TYPE_ID, LEVEL_ID, LINE_ID,  POSTYPE_ID, PER_SALARY FROM PER_PROFILE WHERE POSTYPE_ID = 5  AND PER_STATUS_CIVIL = 2   ");
				...
				while($rec = $db->db_fetch_array($query_per)){
				  ...
				  $query_num = $db->query("SELECT SAL_UP_ID FROM SAL_UP_SALARY WHERE  POSTYPE_ID = 5  AND  YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND PER_ID = '".$rec['PER_ID']."' ");
	...
	case 'Confirm':
		try{	
		   ...
		   if($num_all == 0){
		   		$db->db_update($TB,array('CONFIRM_TYPE' => 2)," POSTYPE_ID = 5 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' "); 
	...
	case 'delete':
		try{	
			...
			$url_back="../setup_per_group_emp_gov_form.php";
	        $text=$del_proc;
		}catch(Exception $e){
			$text=$e->getMessage();
		}
