...
$url_back="../command_spe_salary_emp_gov_disp.php";
$TB = "SAL_COMMAND";
$TB1 = "SAL_UP_SALARY";
...
switch($proc){
		   ...
		   $db->db_insert($TB,$fields);
			...
			if(count($ARR_SAL_UP_ID)>0){
				foreach($ARR_SAL_UP_ID as $key => $val){
						$fields2 = array(
							"SAL_COM_SPE" => $rec_sal_max['SAL_MAX'],
							"CREATE_BY" => $USER_BY,
							"CREATE_DATE" => $TIMESTAMP,
						);	
	...
	case "edit" : 
		try{
			...
			$db->db_update($TB1,array("SAL_COM_SPE" => '')," SAL_COM_ID = '".$SAL_COM_ID."' ");
			if(count($ARR_SAL_UP_ID)>0){
				foreach($ARR_SAL_UP_ID as $key => $val){
						$fields2 = array(
							  "SAL_COM_SPE" => $SAL_COM_ID,
							  "CREATE_BY" => $USER_BY,
							  "CREATE_DATE" => $TIMESTAMP,
						);	
						$db->db_update($TB1,$fields2," SAL_UP_ID = '".$val."'");			
