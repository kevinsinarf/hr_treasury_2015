...
$url_back="../record_salary_emp_frame_disp.php";
$table = "SAL_FRAME";

switch($proc){	
	case 'save' :
		try{
			$db->db_delete("SAL_EVAL"," YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND POSTYPE_ID = 3 AND EVAL_TYPE = 1  ");
			$fields_eval = array(
				"YEAR_BDG" => $S_YEAR_BDG,
				"ROUND" => $S_ROUND,
				"NUM1" => $NUM1,
			...
			$db->db_delete($table," YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND POSTYPE_ID = 3 ");
			if(count($ORG_ID_3) > 0){
	...
	case 'ConfirmGov':
		try{	
		   ...
		   $db->db_update($table,array('CONFIRM_TYPE' => 2)," POSTYPE_ID = 3 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' "); 
