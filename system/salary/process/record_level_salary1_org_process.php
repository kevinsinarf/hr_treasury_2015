...
$url_back="../record_level_salary1_org_form.php";
$table = "SAL_UP_SALARY";
...
switch($proc){	
	...
	case 'ConfirmCom':
		try{	
		   ...
		   $db->db_update($table,array('CONFIRM_TYPE' => 3)," POSTYPE_ID = 1 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND SAL_UP_TYPE = 1 AND ORG_ID_3 = '".$ORG_ID."' "); 
		   $text= 'อนุมัติเกณฑ์การประเมิน เรียบร้อยแล้ว';
	...
	break;
		...
		$query = $db->query("SELECT DISTINCT(SCORE_TYPE) AS SCORE_TYPE, SCORE_ID, SCORE_S,SCORE_E, PERCENT_SAL,PERCENT_SAL_E FROM SAL_SCORE WHERE POSTYPE_ID = '1' AND SCORE_STATUS = 1  AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND ORG_ID_3 = '".$ORG_ID."'  AND '".$score."' BETWEEN SCORE_S AND SCORE_E");
