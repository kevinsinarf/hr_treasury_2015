...
$pk_id="A.ORG_ID";
$wh=" B.POSTYPE_ID = 1 AND B.SAL_UP_TYPE = 1 AND B.YEAR_BDG = '".$S_YEAR_BDG."' AND B.ROUND = '".$S_ROUND."' {$filter} ";
...
<script src="js/setup_per_group_gov_disp.js?<?php echo rand(); ?>"></script>
</head>
<body>
<div class="container-full">
	...
	<div class="col-xs-12 col-sm-12">
		<ol class="breadcrumb">
			...
			<li class="active">ข้าราชการ->จัดกลุ่มสำหรับการเลื่อนเงินเดือน</li>
		</ol>
	</div>
	<div class="col-xs-12 col-sm-12" id="content">
		...
		<form id="frm-search" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            ...
            <div class="row" style="margin-top:10px;">
                <a data-toggle="modal" class="btn btn-default" data-backdrop="static" href="javascript:void(0);" onClick="addPerAllMg();"><?php echo $img_save;?> นำเข้าข้อมูลจากทะเบียนประวัติ (ผู้บริหาร)</a> 
                 <a data-toggle="modal" class="btn btn-default" data-backdrop="static" href="javascript:void(0);" onClick="addPerAll();"><?php echo $img_save;?> นำเข้าข้อมูลจากทะเบียนประวัติ (ผู้ปฏิบัติ)</a> 
               <?php if($nums > 0){ ?>
                 <a data-toggle="modal" class="btn btn-default" data-backdrop="static" href="javascript:void(0);" onClick="ConfirmPer();"><?php echo $img_save;?> อนุมัติการจัดกลุ่มเพื่อกำหนดกรอบเลื่อนเงินเดือน</a>
              <?php } ?>
           </div> 
           <?php } ?>
			<div class="table-responsive">
                <table align="center" class="table table-bordered table-striped table-hover table-condensed" id="tb_data">
					...
					while($rec = $db->db_fetch_array($query)){
						...
						$query_num = $db->query("SELECT COUNT(PER_ID) AS COUNT_PER FROM SAL_UP_SALARY WHERE  POSTYPE_ID = 1 AND SAL_UP_TYPE = 1 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND ORG_ID_3 = '".$rec['ORG_ID']."' ");
