...
$filter = "";
if($YEAR_BDG != ""){
	$filter .= " and YEAR_BDG = '".$YEAR_BDG."' ";
}
if($ROUND != ""){
	$filter .= " and ROUND = '".$ROUND."' ";
}

$field="SAL_COM_ID,YEAR_BDG,ROUND,COM_NO,COM_TITLE,COM_SDATE,COM_DATE";
$table="SAL_COMMAND ";
$pk_id="SAL_COM_ID";
$wh="DELETE_FLAG ='0' AND POSTYPE_MG ='1' {$filter} ";
		...
		<div class="row">
			...
			<div class="col-xs-12 col-sm-2">
				<select name="ROUND" id="ROUND" class="selectbox form-control" placeholder="รอบ">
                    ...
                    for($r=1;$r<=2;$r++){
                        ?>
                       <option value="<?php echo $r;?>" <?php if($r == $ROUND){ echo "selected"; }?>><?php echo $r;?></option>
                        <?php
                    }
                    ?>
                </select>
