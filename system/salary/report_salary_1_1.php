...
if($ROUND == 1){
   $title_1 = " 1 มีนาคม ".($year_fillter_is);
}
if($ROUND == 2){
      $title_1 = " 1 กันยายน ".$year_fillter_is;
}
...
<?php  
   ...
   $sql_org = get_ORG_sql();
	...
	if($AGE_IS > 0){
			...
			$sql_salary = " select NUM_PER,SALARY_NOW from SAL_FRAME   where ORG_ID_3 =".$key."   AND ROUND = '".$ROUND."' AND YEAR_BDG = '".$AGE_IS."' AND CONFIRM_TYPE = 2   ";   // echo $sql_salary; exit();
			...
			$salary_set = ( $all_salary*0.025);
			$salary_cut = ( $all_salary*0.005);			
		}      
		
		$html  .= "<tr  ".$tr_detail_style." >";
		$html  .= "<td CENTER_TOP  >".toThaiNumber($start_no)."</td>"; 
		$html  .= "<td LEFT_TOP   >&nbsp;&nbsp;".toThaiNumber($value)."</td>";
		$html  .= "<td CENTER_TOP >".number_format_thai($all_people,0)."&nbsp;</td>"; 
		$html  .= "<td RIGHT_TOP >".number_format_thai($all_salary,2)."&nbsp;</td>"; 
		$html  .= "<td RIGHT_TOP >".number_format_thai($salary_slide,2)."&nbsp;</td>"; 
		$html  .= "<td RIGHT_TOP >".number_format_thai($salary_set,2)."&nbsp;</td>"; 
		$html  .= "<td RIGHT_TOP >".number_format_thai($salary_cut,2)."&nbsp;</td>";
		$html  .= "</tr>";
		 $start_no++;
	}
    //if($all_people_sum>0){
		...
		$html  .= "<td CENTER_TOP >".number_format_thai($all_people_sum,0)."&nbsp;</td>"; 
		$html  .= "<td RIGHT_TOP >".number_format_thai($all_salary_sum,2)."&nbsp;</td>"; 
		$html  .= "<td RIGHT_TOP >".number_format_thai($salary_slide_sum,2)."&nbsp;</td>"; 
		$html  .= "<td RIGHT_TOP >".number_format_thai($salary_set_sum,2)."&nbsp;</td>"; 
		$html  .= "<td RIGHT_TOP >".number_format_thai($salary_cut_sum,2)."&nbsp;</td>";
		$html  .= "</tr>";
	//	}
$SEARCH_TYPE = $title_1;
