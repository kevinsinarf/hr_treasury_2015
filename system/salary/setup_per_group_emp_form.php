...
$pk_id="A.SAL_UP_ID";
$wh=" A.POSTYPE_ID = 3 AND A.YEAR_BDG = '".$S_YEAR_BDG."' AND A.ROUND = '".$S_ROUND."' AND A.ORG_ID_3 = '".$ORG_ID_3."' {$filter} ";
$groupby = "  "; 
...
$query_all = $db->query("SELECT COUNT(PER_ID) AS COUNT_PER FROM SAL_UP_SALARY WHERE  POSTYPE_ID = 3 AND CONFIRM_TYPE >= 2 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' ");
...
<script src="js/setup_per_group_emp_form.js?<?php echo rand(); ?>"></script>
</head>
<body>
<div class="container-full">
	...
	<div class="col-xs-12 col-sm-12">
		<ol class="breadcrumb">
			...
			<li class="active"><a href="#" onClick=" $('#page').val(1); $('#frm-search').attr('action','setup_per_group_emp_disp.php').submit(); ">พนักงานราชการ->จัดกลุ่มสำหรับการเลื่อนค่าตอบแทน</a></li>
            	...
            	<div class="row">
						...
						<div class="col-xs-12 col-sm-2">
							<select id="S_TYPE_ID" name="S_TYPE_ID" class="selectbox form-control" placeholder="-ทั้งหมด-" onChange="get_level(this); ">
								<option value=""></option>
								<?php
								$sql_type_name = "Select TYPE_ID , TYPE_NAME_TH From SETUP_POS_TYPE WHERE ACTIVE_STATUS = 1 AND POSTYPE_ID = '3' AND DELETE_FLAG = 0 ORDER BY TYPE_SEQ ASC";
                      ...
                      <div class="col-xs-12 col-sm-4 col-md-2">
                          <select id="S_LEVEL_ID" name="S_LEVEL_ID" class="selectbox form-control" placeholder="-ทั้งหมด-" onChange="get_line(this);" >
                              <option value=""></option>
                              <?php
                             
							  $cond_level =  " and TYPE_ID = '".$S_TYPE_ID."'";
                              $sql_level_name = "Select LEVEL_ID , LEVEL_NAME_TH  From SETUP_POS_LEVEL WHERE ACTIVE_STATUS = 1 AND POSTYPE_ID = '3' AND DELETE_FLAG = '0'".$cond_level." ORDER BY LEVEL_SEQ ASC";
			    ...
			    <div class="row">
                      ...
                      <div class="col-xs-12 col-sm-4 col-md-2">                                        
                          <select id="S_LINE_ID" name="S_LINE_ID" class="selectbox form-control" placeholder="-ทั้งหมด-" >
                              <option value=""></option>
                              <?php
                              $sql_line_name="SELECT LINE_ID , LINE_NAME_TH FROM SETUP_POS_LINE WHERE DELETE_FLAG='0' AND POSTYPE_ID = '3' AND LG_ID = '".$S_LEVEL_ID."' ORDER BY LINE_NAME_TH ASC";
                      ...
                      <div class="col-xs-12 col-sm-4 col-md-2">
                          <select id="S_ORG_ID_4" name="S_ORG_ID_4" class="selectbox form-control" placeholder="-ทั้งหมด-" >
                              <option value=""></option>
                              <?php
                              $sql_org_4 = "Select ORG_ID , ORG_NAME_TH From SETUP_ORG WHERE ACTIVE_STATUS = 1 AND ORG_PARENT_ID = '".$ORG_ID_3."' AND DELETE_FLAG = '0' ORDER BY ORG_SEQ ASC";
                ...
                <table align="center" class="table table-bordered table-striped table-hover table-condensed" id="tb_data">
                    <thead>
                        <tr class="bgHead">
                            ...
                            <th width="13%"><div align="center"><strong>ตำแหน่ง</strong></div></th>
                            <th width="10%"><div align="center"><strong>ประเภทกลุ่มงาน</strong></div></th>
