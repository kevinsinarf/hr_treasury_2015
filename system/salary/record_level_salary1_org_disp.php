...
$sql_org = "SELECT A.ORG_ID_3, B.ORG_NAME_TH FROM SAL_FRAME A
				JOIN SETUP_ORG B ON A.ORG_ID_3 = B.ORG_ID
				WHERE A.YEAR_BDG = '".$S_YEAR_BDG."' AND A.ROUND = '".$S_ROUND."' AND A.POSTYPE_ID = 1 AND A.CONFIRM_TYPE = 2
 ...
 $txt_error = '';
if($nums == 0){
	$txt_error = "<div class='row' style='color:red'>**กรุณาอนุมัติกรอบวงเงิน (ผู้ปฏิบัติ) ก่อน</div>";
}
...
<script src="js/record_level_salary1_org_disp.js?<?php echo rand(); ?>"></script>
</head>
<body>
<div class="container-full">
	...
	<div class="col-xs-12 col-sm-12">
		<ol class="breadcrumb">
			...
			<li class="active">บันทึกการเลื่อนขั้นเงินเดือนข้าราชการตามสำนัก->ข้าราชการ->บันทึกผล (ผู้ปฏิบัติงาน)</li>
		</ol>
	</div>
			...
			<div class="table-responsive">
                <table align="center" class="table table-bordered table-striped table-hover table-condensed" id="tb_data">
					...
					while($rec_org = $db->db_fetch_array($query_org)){
						$query_frame = $db->query("SELECT NUM_PER, SALARY_NOW FROM SAL_FRAME WHERE POSTYPE_ID = 1 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND ORG_ID_3 = '".$rec_org['ORG_ID_3']."' ");
						$rec_frame = $db->db_fetch_array($query_frame);
						
						$query_score = $db->query("SELECT COUNT(SCORE_ID) AS NUM FROM SAL_SCORE WHERE POSTYPE_ID = 1 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND ORG_ID_3 = '".$rec_org['ORG_ID_3']."' AND SCORE_STATUS = 1 AND CONFIRM_TYPE >= 2 ");
						...
						$SALARY_UP = $db->get_data_field("SELECT SUM(SALARY_UP) AS SALARY_UP FROM SAL_UP_SALARY WHERE YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND ORG_ID_3 = '".$rec_org['ORG_ID_3']."' AND POSTYPE_ID = '1' AND SAL_UP_TYPE = 1", "SALARY_UP");
