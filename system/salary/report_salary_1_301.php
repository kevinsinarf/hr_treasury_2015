...
<?php  
   $arr_org = array();
$where = " WHERE A.YEAR_BDG = '".$AGE_IS."' AND A.ROUND = '".$ROUND."' AND  A.SALARY_SPE_UP > 0   AND A.POSTYPE_ID =5   AND A.CONFIRM_TYPE = 3 AND A.SAL_UP_TYPE = 2    AND  M.CONFIRM_TYPE = 2  "; 
$sql =  attached_sql($where);
	 ...
	 //echo "<pre>"; print_r($row);
					...
					if($nums > 0){
						...
						while($rec = $db->db_fetch_array($query)){
							...
							$PERCENT_SPE = toThaiNumber(number_format($rec['PERCENT_SPE'],2));
							$LEVEL_SALARY_MAX = toThaiNumber(number_format($rec['PERCENT_SPE'],2));
							$percent_2 = "-";
							$percent_4 = "-";
							$percent_6 = "-";
							if($PERCENT_SPE == 2){
							   $percent_2 =  $SALARY_SPE_UP;
							}
							if($PERCENT_SPE == 4){
							   $percent_4 =  $SALARY_SPE_UP;
							}
							if($PERCENT_SPE == 6){
							   $percent_6 =  $SALARY_SPE_UP;
							}
						  ...
						  $html .= "<tr style='background-color:#fff;'  > 
							...
							<td RIGHT_TOP >".$LEVEL_SALARY_MAX."</td>
							<td RIGHT_TOP >".$percent_2."</td>
							<td RIGHT_TOP >".$percent_4."</td>
							<td RIGHT_TOP >".$percent_6."</td>
