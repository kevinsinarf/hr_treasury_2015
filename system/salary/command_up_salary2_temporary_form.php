...
if($proc == 'edit'){
 ...
 $sql = "SELECT B.PER_IDCARD,B.PREFIX_ID,B.PER_ID,B.PER_FIRSTNAME_TH,B.PER_MIDNAME_TH, B.PER_LASTNAME_TH ,B.POS_NO,
			...
			WHERE  SAL_UP_TYPE = 1 AND CONFIRM_TYPE = 3 AND A.YEAR_BDG = '".$YEAR_BDG."' AND ROUND = '".$ROUND."' AND A.POSTYPE_ID = 1 AND A.SALARY_NOW < 12285   ";
$query = $db->query($sql);
$nums = $db->db_num_rows($query);

$arr_command_type = GetSqlSelectArray("CT_ID", "CT_NAME_TH", "SETUP_COMMAND_TYPE", " LT_ID = 8 AND ACTIVE_STATUS = 1 ", "CT_NAME_TH");
$arr_movement = GetSqlSelectArray("MOVEMENT_ID", "MOVEMENT_NAME_TH", "SETUP_MOVEMENT", " ACTIVE_STATUS = '1' AND DELETE_FLAG = '0' AND POSTYPE_ID = '1' AND MOVEMENT_ID = 17 ", "MOVEMENT_NAME_TH");
?>
<!DOCTYPE html>
<html>
<head>
...
<script src="js/command_up_salary_2_temporary_form.js?<?php echo rand(); ?>"></script>
</head>
<body>
<div class="container-full">
    ...
    <div class="col-xs-12 col-sm-12">
        <ol class="breadcrumb">
          ...
          <li><a href="command_up_salary2_temporary_disp.php?<?php echo url2code("menu_id=".$menu_id."&menu_sub_id=".$menu_sub_id);?>">จัดทำคำสั่ง->ข้าราชการ-><?php echo Showmenu($menu_sub_id);?></a></li>
          <li class="active"><?php echo $txt;?></li>
        </ol>
    </div>
    <div class="col-xs-12 col-sm-12" id="content">
        <div class="groupdata" >
		<form id="frm-input" method="post" action="process/command_up_salary2_temporary_process.php">
        ...
        <div class="row formSep">
         ...
         <div class="row formSep">
            ...
            <div class="col-xs-12 col-md-3">
			  <?php echo text($arr_movement['17']); ?>
              <input type="hidden" id="MOVEMENT_ID" name="MOVEMENT_ID" value="17">
            </div> 
        </div>
		<div class="row formSep">
            ...
            <?php if($num_com == 0){ ?>
			  <button type="button" class="btn btn-primary" onClick="chkinput();">บันทึก</button>
               <?php if($num_all > 0){ ?>
                ...
                <?php } ?>
            <?php } ?>
              <button type="button" class="btn btn-default" onClick="$('#frm-input').attr('action','command_up_salary2_temporary_disp.php').submit();">ยกเลิก</button>
			</div>
        </div>
      </form>
    </div>
