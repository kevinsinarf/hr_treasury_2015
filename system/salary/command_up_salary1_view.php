...
WHERE A.MANAGE_ID= '".$MANAGE_ID."' AND A.MANAGE_ID > 0";

$query = $db->query($sql);
$nums = $db->db_num_rows($query);
$total_record = $db->db_num_rows($db->query("SELECT A.YEAR_BDG, A.ROUND, B.COM_TITLE, B.COM_NO, B.COM_SDATE, B.COM_DATE, C.PER_FIRSTNAME_TH, C.PER_MIDNAME_TH, C.PER_LASTNAME_TH, D.ORG_NAME_TH, A.SALARY_NEW, E.LEVEL_NAME_TH, G.LINE_NAME_TH 
FROM SAL_UP_SALARY A
LEFT JOIN SAL_COMMAND B ON A.SAL_COM_ID = B.SAL_COM_ID
LEFT JOIN PER_PROFILE C ON A.PER_ID = C.PER_ID
LEFT JOIN SETUP_ORG D ON A.ORG_ID_3 = D.ORG_ID
...
$sql_org = "SELECT A.MANAGE_ID,B.MANAGE_NAME_TH FROM SAL_UP_SALARY A LEFT JOIN SETUP_POS_MANAGE B ON A.MANAGE_ID = B.MANAGE_ID WHERE A.MANAGE_ID= '".$MANAGE_ID."'";
...
<div class="container-full">
	...
	<div class="col-xs-12 col-md-12">
        <ol class="breadcrumb">
          ...
          <li><a href="command_up_salary1_disp.php?<?php echo url2code("menu_id=".$menu_id."&menu_sub_id=".$menu_sub_id."&YEAR_BDG=".$YEAR_BDG."&ROUND=".$ROUND."&proc=".$proc."&SAL_COM_ID=".$SAL_COM_ID);?>"><?php echo Showmenu($menu_sub_id);?></a></li>
          <li><a href="command_up_salary1_form.php?<?php echo url2code("menu_id=".$menu_id."&menu_sub_id=".$menu_sub_id."&YEAR_BDG=".$YEAR_BDG."&ROUND=".$ROUND."&proc=".$proc."&SAL_COM_ID=".$SAL_COM_ID);?>"><?php echo $txt; ?></a></li>
	...
	<div class="col-xs-12 col-md-12">
		<div class="groupdata">
			<form id="frm-search" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                ...
                <div class="row">
                	<div class="col-xs-12 col-md-2"><strong>ตำแหน่งทางการบริหาร :</strong></div>
                    <div class="col-xs-12 col-sm-2"><?php echo text($rec_org['MANAGE_NAME_TH']);?></div>
                </div>
					...
					<div class="table-responsive">
                        <table width="397" class="table table-bordered table-striped table-hover table-condensed">
                          <thead>
                              ...
                              <th width="15%"  style="text-align:center; vertical-align:middle" ><strong>เงินเดือน</strong></th>
                           </tr>
                           </thead>
							<tbody>
