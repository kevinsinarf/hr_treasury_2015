...
if($proc == 'edit'){
	...
	$query_sal_all = $db->query("SELECT COUNT(SAL_UP_ID) AS NUM_SAL FROM SAL_UP_SALARY WHERE  SAL_COM_ID = '".$SAL_COM_ID."' ");
	...
	$num_com = $rec_com['NUM_COM'];
}

 $sql = "SELECT B.PER_IDCARD,B.PREFIX_ID,B.PER_ID,B.PER_FIRSTNAME_TH,B.PER_MIDNAME_TH, B.PER_LASTNAME_TH ,B.POS_NO,
			B.PER_SALARY,A.SAL_COMPENSATION_4,A.SALARY_NEW,A.SAL_UP_ID, TYPE_NAME_TH,LEVEL_NAME_TH,LINE_NAME_TH, F.ORG_NAME_TH AS ORG_NAME_3, 
			...
			WHERE SAL_UP_TYPE = 1 AND CONFIRM_TYPE = 3 AND  A.YEAR_BDG = '".$YEAR_BDG."' AND ROUND = '".$ROUND."' AND A.POSTYPE_ID = 1  AND SAL_UP_TYPE = 1 AND A.CONFIRM_TYPE = 3 ";
$query = $db->query($sql);
$nums = $db->db_num_rows($query);

$arr_command_type = GetSqlSelectArray("CT_ID", "CT_NAME_TH", "SETUP_COMMAND_TYPE", "LT_ID = 8 AND ACTIVE_STATUS = 1 ", "CT_NAME_TH");
$arr_movement = GetSqlSelectArray("MOVEMENT_ID", "MOVEMENT_NAME_TH", "SETUP_MOVEMENT", " ACTIVE_STATUS = '1' AND DELETE_FLAG = '0' AND POSTYPE_ID = '1' AND MOVEMENT_ID = '15'  ", "MOVEMENT_NAME_TH");
?>
<!DOCTYPE html>
<html>
<head>
...
<script src="js/command_up_salary2_form2.js?<?php echo rand(); ?>"></script>
</head>
<body>
<div class="container-full">
    ...
    <div class="col-xs-12 col-sm-12">
        <ol class="breadcrumb">
          ...
          <li><a href="command_up_salary2_disp.php?<?php echo url2code("menu_id=".$menu_id."&menu_sub_id=".$menu_sub_id);?>">จัดทำคำสั่ง->ข้าราชการ-><?php echo Showmenu($menu_sub_id);?></a></li>
          <li class="active"><?php echo $txt;?></li>
        </ol>
    </div>
    <div class="col-xs-12 col-sm-12" id="content">
        <div class="groupdata" >
		<form id="frm-input" method="post" action="process/command_up_salary2_process2.php">
        ...
        <div class="row formSep">
            ...
            <div class="col-xs-12 col-sm-3"><?php echo $arr_round[$ROUND]; ?></div>
        </div>
         <div class="row formSep">
            ...
            <div class="col-xs-12 col-md-3">
			  <?php echo text($arr_movement['15']); ?>
              <input type="hidden" id="MOVEMENT_ID" name="MOVEMENT_ID" value="15">
            </div> 
        </div>
		<div class="row formSep">
        ...
        <div class="row formSep">
                      ...
                      <table align="center" class="table table-bordered table-striped table-hover table-condensed" id="tb_data">
                        <thead>
                            <tr class="bgHead">
                                ...
                                <th width="10%"><div align="center"><strong>เงินเดือนเดิม</strong></div></th> 
                                <th width="10%"><div align="center"><strong>เงินเดือนใหม่</strong></div></th>  
                            </tr>
                         <?php
						 if($nums > 0){
							$i = 1;
							...
							while($rec = $db->db_fetch_array($query)){
								...
								$POS_DETAIL .= "<br><strong>ตำแหน่งทางการบริหาร(ถ้ามี) : </strong>".text($rec['MANAGE_NAME_TH']);
								...
								if($proc == 'edit'){
									$query_sal_com = $db->query("SELECT SAL_UP_ID FROM SAL_UP_SALARY WHERE SAL_UP_ID = '".$rec['SAL_UP_ID']."' AND SAL_COM_ID = '".$SAL_COM_ID."' ");
			...
			<div class="col-xs-12 col-md-12" align="center">
              <?php if($num_com == 0){ ?>
                ...
                <?php } ?>
              <?php } ?>
              <button type="button" class="btn btn-default" onClick="$('#frm-input').attr('action','command_up_salary2_disp.php').submit();">ยกเลิก</button>
			</div>
        </div>
      </form>
    </div>
