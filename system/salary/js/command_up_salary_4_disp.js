...
function addData(){
	...
	$("#proc").val("add");
	$("#frm-search").attr("action", "command_up_salary4_form.php" ).submit();
}
...
function View(id){
	if(confirm('คุณต้องการบันทึกก่อนดูรายละเอียดหรือไม่ เนื่องจากระบบยังไม่ได้บันทึกข้อมูลคำสั่งที่ท่านกรอก')){
		$("#ORG_ID_3").val(id);
		$("#frm-input").attr("action", "command_up_salary4_view.php" ).submit();
	}
}
...
function delData(id){
	if(confirm("ต้องการลบข้อมูลใช่หรือไม่ ?")){
		$("#proc").val("delete");
		$("#SAL_COM_ID").val(id);
		$("#frm-search").attr("action","process/command_up_salary4_process.php").submit();
	}
}
