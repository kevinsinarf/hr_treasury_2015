...
function editData(id){
	...
	$("#proc").val("edit");
	$("#ORG_ID").val(id);
	$("#frm-search").attr("action","record_level_salary1_org_form.php").submit();
}
function getPercent(score, id_tb){
	var url = 'process/record_level_salary1_org_process.php';
	var TYPE_ID = $('#TYPE_ID_'+id_tb).val();
	var LEVEL_ID = $('#LEVEL_ID_'+id_tb).val();
	var LINE_ID = $('#LINE_ID_'+id_tb).val();
	var ORG_ID = $('#ORG_ID').val();
	
	$.post(url,{proc: 'getPercent',score: score, id_tb: id_tb, S_YEAR_BDG : $('#S_YEAR_BDG').val(), S_ROUND : $('#S_ROUND').val(),'TYPE_ID':TYPE_ID, 'LEVEL_ID':LEVEL_ID, 'LINE_ID':LINE_ID, 'ORG_ID':ORG_ID}
	,function(data){	
...
function getSalaryCal(id_tb){
	...
	if(SALARY>MAX){
		...
		if(SALARY_NOW >= MAX){	
		  //SPAC_COM =  SALARY-MAX;	
		  $('#shw_salary_new_'+id_tb).html(number_format_return(MAX,2));
		  ...
		  $("#shw_salary_spe_new_"+id_tb).html(number_format_return(SALARY_UP,2));
			...
			$('#shw_salary_new_'+id_tb).html(number_format_return(MAX,2));
		 	$('#SHW_NEW_'+id_tb).val(number_format_return(MAX,2));
		  	$("#SALARY_SPE_UP_"+id_tb).val(number_format_return(SALARY_UP,2));  
		  	$("#shw_salary_spe_new_"+id_tb).html(number_format_return(SALARY_UP,2));
		}
	}else if(SALARY<=MAX){
