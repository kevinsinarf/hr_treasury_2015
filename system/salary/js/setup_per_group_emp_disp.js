...
function addGroup(id){
	...
	$('#page').val(1);
	$('#OGR_ID_3').val(id);
	$('#frm-search').attr('action','setup_per_group_emp_form.php');
	$('#frm-search').submit();
}
function addPerAll(){
	if($('#S_YEAR_BDG').val() == ''){
	...
	if(confirm("การนำเข้าข้อมูลใหม่ ระบบจะทำการลบข้อมูลการจัดกลุ่มเดิมทั้งหมด คุณต้องการดำเนินการต่อใช้หรือไม่ ?")){
	  $('#proc').val('AddAll');
	  $('#frm-search').attr('action','process/setup_per_group_emp_process.php');
	  $('#frm-search').submit();
	}
}
function ConfirmPer(){
