...
function addData(){
	...
	$("#proc").val("add");
	$("#frm-search").attr("action", "command_up_salary1_form.php" ).submit();
}
...
function View(id){
	$("#MANAGE_ID").val(id);
	$("#frm-input").attr("action", "command_up_salary1_view.php" ).submit();
}
...
function delData(id){
	if(confirm("ต้องการลบข้อมูลใช่หรือไม่ ?")){
		$("#proc").val("delete");
		$("#SAL_COM_ID").val(id);
		$("#frm-search").attr("action","process/command_up_salary1_process.php").submit();
	}
}
