...
function AddTransfer(){
	...
	$('#frm-transfer').attr('action','process/setup_per_group_emp_gov_process.php');
	$('#frm-transfer').submit();
}
function delData(id){
	if(confirm("ต้องการลบข้อมูลใช่หรือไม่ ?")){
		$("#proc").val("delete");
		$("#SAL_UP_ID").val(id);
		$("#frm-search").attr("action","process/setup_per_group_emp_gov_process.php").submit();
	}
}
function get_level(e){
	...
	if(e.value > 0 && $.trim(e.value) != ""){
		$.ajax({
			url: 'process/setup_per_group_emp_gov_process.php',
			dataType : 'json',
			type: "POST",
