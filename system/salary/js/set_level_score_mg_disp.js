...
function ConfirmMg(){
	...
	if(confirm("อนุมัติเกณฑ์การประเมิน\nประจำปีงบประมาณ "+S_YEAR_BDG+" รอบ "+S_ROUND+"\nคุณจะไม่สามารถแก้ไขข้อมูลได้อีก\nคุณต้องการดำเนินการต่อใช้หรือไม่ ?")){
		$('#proc').val('ConfirmMg');
		$("#frm-search").attr('action','process/set_level_score_mg_process.php').submit();
	}
}
function add_row(id){
	var x = '';
	...
	if(id==1||id==3){
		...
		var score_type = x+"<input type=\"text\" id=\"PERCENT_SAL_E"+id_tb+"\" name=\"PERCENT_SAL_E[]\"  class=\"form-control\" maxlength=\"6\" style=\"width:100px;display:inline; text-align:center;\" onKeyUp=\"chkFormatNam_id(this.value,this.id);\" onBlur=\"NumberFormat(this,2)\">";
	}else{
		var score_type = "";
	}
	table.rows[rowCount].cells[2].innerHTML="<input type=\"text\" id=\"SCORE_S_"+id_tb+"\" name=\"SCORE_S[]\"  class=\"form-control\" maxlength=\"6\" style=\"width:100px;display:inline; text-align:center;\" onKeyUp=\"chkFormatNam_id(this.value,this.id);\" onBlur=\"NumberFormat(this,2)\">-<input type=\"text\" id=\"SCORE_E_"+id_tb+"\" name=\"SCORE_E[]\"  class=\"form-control\" maxlength=\"6\" style=\"width:100px;display:inline; text-align:center;\" onKeyUp=\"chkFormatNam_id(this.value,this.id);\" onBlur=\"NumberFormat(this,2)\">";
	table.rows[rowCount].cells[3].innerHTML="<input type=\"text\" id=\"PERCENT_SAL_"+id_tb+"\" name=\"PERCENT_SAL[]\"  class=\"form-control\" maxlength=\"6\" style=\"width:100px;display:inline; text-align:center;\" onKeyUp=\"chkFormatNam_id(this.value,this.id);\" onBlur=\"NumberFormat(this,2)\">"+score_type;
