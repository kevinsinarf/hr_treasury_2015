...
function chkinput(){
	...
	if($('input:checkbox[name^=SAL_UP_ID]:checked').length <= 0){
		alert('กรุณาเลือก ชื่อผู้ได้เลื่อนเงินเดือน อย่างน้อย 1 รายการ');
		$('#chkAll').focus();
		return false;
	}
...
function ConfirmCom(){
	...
	if(confirm("อนุมัติออกคำสั่งให้ได้เลื่อนค่าตอบแทน\nปีงบประมาณ "+YEAR_BDG+" รอบ "+ROUND+"\nคุณจะไม่สามารถแก้ไขข้อมูลได้อีก\nคุณต้องการดำเนินการต่อใช้หรือไม่ ?")){
		$('#proc').val('ConfirmCom');
		$("#frm-input").attr('action','process/command_up_salary3_process.php').submit();
	}
}
// =======================================
