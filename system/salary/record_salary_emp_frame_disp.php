...
$query_num = $db->query("SELECT COUNT(*) AS NUM FROM SAL_UP_SALARY WHERE POSTYPE_ID = 3 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND SAL_UP_TYPE = 1 AND CONFIRM_TYPE >= 2  ");
$rec_num = $db->db_fetch_array($query_num);
$num_group = $rec_num['NUM'];
$NUM1_CAL = $db->get_data_field("SELECT NUM1 FROM SAL_EVAL WHERE YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND POSTYPE_ID = 3 AND EVAL_TYPE = 1 ", "NUM1");

$query_org = $db->query("SELECT A.ORG_ID_3, B.ORG_NAME_TH 
                                   FROM SAL_UP_SALARY A 
								   JOIN SETUP_ORG B ON A.ORG_ID_3 = B.ORG_ID 
								   WHERE A.POSTYPE_ID = 3 AND A.YEAR_BDG = '".$S_YEAR_BDG."' AND A.ROUND = '".$S_ROUND."' AND A.SAL_UP_TYPE = 1 AND A.CONFIRM_TYPE >= 2
...
$query_all = $db->query("SELECT COUNT(SAL_FRAME_ID) AS COUNT_FRAME FROM SAL_FRAME WHERE POSTYPE_ID = 3 AND CONFIRM_TYPE = 2 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."'  ");
...
<script src="js/record_salary_emp_frame_disp.js?<?php echo rand(); ?>"></script>
</head>
<body>
<div class="container-full">
	...
	<div class="col-xs-12 col-sm-12">
		<ol class="breadcrumb">
			...
			<li class="active">พนักงานราชการ-><?php echo Showmenu($menu_sub_id);?></li>
		</ol>
	</div>
		...
		<form id="frm-search" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
			...
			<div class="row">
                ...
                <select name="S_ROUND" id="S_ROUND" class="selectbox form-control" placeholder="รอบ" onChange="$('#frm-search').submit();">
                    <option value=""></option>
					<?php 
                   foreach($arr_emp_round as $index => $val){
                        ?>
                    ...
                    <thead>
                        <tr class="bgHead">
                            ...
                            <th width="15%"><div align="center"><strong>จำนวนคน</strong></div></th>                                                            
                          	<th width="20%"><div align="center"><strong>ค่าตอบแทน</strong></div></th>
                          	<th width="20%"><div align="center"><strong>ค่าตอบแทน x <input type="text" name="NUM1" id="NUM1" value="<?php echo number_format($NUM1,2);?>" class="form-control" style="width:70px; display:inline;" onBlur="NumberFormat(this,2)" maxlength="5" onKeyUp="calSalary(this.value); chkFormatNam_id(this.value,this.id);" ></strong></div></th>
                        </tr>
                    </thead>
                    <tbody >
                    <?php
					if($nums > 0){
					  ...
					  while($rec_org = $db->db_fetch_array($query_org)){
						  ...
						  $sql_per_group = "SELECT COUNT(A.PER_ID) AS COUNT_PER, SUM(A.SALARY_NOW) AS SUM_SALARY
												   FROM SAL_UP_SALARY A 
												   WHERE A.POSTYPE_ID = 3 AND A.YEAR_BDG = '".$S_YEAR_BDG."' AND A.ROUND = '".$S_ROUND."' AND A.ORG_ID_3 = '".$rec_org['ORG_ID_3']."' AND SAL_UP_TYPE = 1 ";
						  $query_per_group = $db->query($sql_per_group);
						  $rec_per_group = $db->db_fetch_array($query_per_group);
						  $query_frame = $db->query("SELECT * FROM SAL_FRAME WHERE POSTYPE_ID = 3 AND YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND ORG_ID_3 = '".$rec_org['ORG_ID_3']."'   ");
