...
$query_level = $db->query("SELECT A.LEVEL_ID, A.LEVEL_NAME_TH, B.TYPE_NAME_TH FROM SETUP_POS_LEVEL A JOIN SETUP_POS_TYPE B ON A.TYPE_ID = B.TYPE_ID WHERE A.LEVEL_ID IN(2,6,8,9) ORDER BY B.TYPE_SEQ DESC, A.LEVEL_SEQ DESC ");

$NUM1_CAL = $db->get_data_field("SELECT NUM1 FROM SAL_EVAL WHERE YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND POSTYPE_ID = 1 AND EVAL_TYPE = 2 ", "NUM1");
$query_nums = $db->query("SELECT COUNT(MG_FRAME_ID) AS CONT_FRAME FROM SAL_FRAME_MG WHERE YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND CONFIRM_TYPE = 2 ");
$rec_nums = $db->db_fetch_array($query_nums);
$nums = (int)$rec_nums['CONT_FRAME'];
...
<script src="js/record_salary_gov_frame_mg_disp.js?<?php echo rand(); ?>"></script>
</head>
<body>
		...
		<form id="frm-search" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                ...
                <table align="center" class="table table-bordered table-striped table-hover table-condensed" id="tb_data">
                    <thead>
                        <tr class="bgHead">
                            <th width="5%"><div align="center"><strong>ลำดับ</strong></div></th>
                          	<th width="20%"><div align="center"><strong>ประเภทตำแหน่ง</strong></div></th> 
                    ...
                    <?php
					  ...
					  while($rec_level = $db->db_fetch_array($query_level)){
						  ...
						  $NUM_PER = 0;
						  $COLOR = '';
						  $sql_per = "SELECT COUNT(A.PER_ID) AS COUNT_PER, SUM(A.SALARY_NOW) AS SUM_SALARY
												   FROM  SAL_UP_SALARY A 
												   WHERE A.POSTYPE_ID = 1 AND A.SAL_UP_TYPE = 2 AND A.LEVEL_ID = '".$rec_level['LEVEL_ID']."' AND A.YEAR_BDG = '".$S_YEAR_BDG."' AND A.ROUND = '".$S_ROUND."' ";
						  $query_per = $db->query($sql_per);
						  $rec_per = $db->db_fetch_array($query_per);
						  $query_frame = $db->query("SELECT * FROM SAL_FRAME_MG WHERE  YEAR_BDG = '".$S_YEAR_BDG."' AND ROUND = '".$S_ROUND."' AND LEVEL_ID = '".$rec_level['LEVEL_ID']."' ");
						  ...
						  if((float)$rec_frame['SALARY_FRAME'] > 0){
							  ...
							  $COLOR = 'color:red;';
							  $SALARY_FRAME =  ($rec_per['SUM_SALARY']*$NUM1)/100;
						  }
						  if((float)$rec_frame['SALARY_NOW'] > 0){
							  $SALARY_NOW = $rec_frame['SALARY_NOW'];
						  }else{
							  $SALARY_NOW = $rec_per['SUM_SALARY'];
						  }
						  if((int)$rec_frame['NUM_PER'] > 0){
							  $NUM_PER = $rec_frame['NUM_PER'];
						  }else{
							  $NUM_PER = $rec_per['COUNT_PER'];
						  }
						  
						  //$salary_cal = ($r_org['SALARY']*$NUM1)/100;
						?>
                        <input type="hidden" name="LEVEL_ID[<?php echo $i;?>]" id="LEVEL_ID_<?php echo $i;?>" value="<?php echo $rec_level['LEVEL_ID'];?>">
            ...
            <div class="formlast">
                <div class="col-xs-12 col-sm-12" align="center">
                    ...
                    <button type="button" class="btn btn-default" onClick="ConfirmMg();">อนุมัติการบันทึกกรอบวงเงิน</button>
                </div>
            </div>
            <?php } ?>                              
		</form>
