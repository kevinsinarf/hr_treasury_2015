...
include_once("inc_year_round.php");  
 ...
 $sql =  attached_sql("  WHERE A.SAL_UP_TYPE = 1 AND A.CONFIRM_TYPE = 3 AND  A.YEAR_BDG = '".$AGE_IS."' AND A.ROUND = '".$ROUND."' AND A.POSTYPE_ID = 1  AND A.SAL_UP_TYPE = 1 AND A.CONFIRM_TYPE = 3 AND A.SAL_COM_ID > 0 ");
 
 //" WHERE A.YEAR_BDG = '".$AGE_IS."' AND A.ROUND = '".$ROUND."' AND  A.SALARY_UP > 0 AND A.POSTYPE_ID =1 AND A.CONFIRM_TYPE = 3 AND A.SAL_UP_TYPE = 2    AND  K.CONFIRM_TYPE = 2 ");
						...
						while($rec = $db->db_fetch_array($query)){
						   ...
						   if($ORG_NAME=="-"){ $ORG_NAME = "&nbsp;"; }
							  $html .= " <tr style='background-color:#fff;'  > 
										...
										<td CENTER_TOP >   </td>
										<td CENTER_TOP >   </td>
										<td  LEFT_TOP colspan=2  ><u><strong>".$ORG_NAME."</strong></u></td>
										<td CENTER_TOP >   </td>
										<td CENTER_TOP >   </td>
										<td CENTER_TOP >   </td>
										<td CENTER_TOP >   </td>
						  ...
						  $html .= "<tr style='background-color:#fff;'  > 
							...
							<td LEFT_TOP >".toThaiNumber($LEVEL_SCORE_NAME)."</td>
				 
							<td RIGHT_TOP >".toThaiNumber(number_format($SCORE_PERCENT,2))."</td>
							<td RIGHT_TOP >".toThaiNumber(number_format($PERCENT,3))."</td>
