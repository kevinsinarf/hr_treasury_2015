...
$SALHIS_UP =  str_replace(',','',$_POST['SALHIS_UP']);
if($POSTYPE_ID==3){
...
switch($proc){
	case "add" : 
		try{		
			...
			$fields = array(
				...
				'COMPENSATION_3' =>  str_replace(",","",$COMPENSATION_3),
				'COMPENSATION_4' =>  str_replace(",","",$COMPENSATION_4),
	...
	break;
	case "GetManage" : {
		$sql = "Select MANAGE_ID , MANAGE_NAME_TH From SETUP_POS_MANAGE WHERE DELETE_FLAG = '0' AND MT_ID = '".$MT_ID."' ORDER BY MANAGE_ID ";
		$query = $db->query($sql);
		$obj = array();
		while($rec = $db->db_fetch_array($query)){
