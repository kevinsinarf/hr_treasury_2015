...
switch($proc){
	...
	case "rampr" : //อำเภอ
		try{
			...
			$name_tamb=empty($_POST['name_tamb'])?'s_tamb'.$key_index:$_POST['name_tamb'];
			$val=$_POST['val'];
			$id_shw_tamb=$_POST['id_shw_tamb'];
			...
			//echo ($z_id);
			echo GetHtmlSelect($z_id,$z_name,$arr_ampr,'อำเภอ/เขต',$val,'onchange="getStamb(this.id,this.value,\''.$name_tamb.'\',\''.$id_shw_tamb.'\')"','','1', '200');
		}catch(Exception $e){
			$text=$e->getMessage();
		}
	break;
	...
	case "stamb" : //ตำบล
		try{
			...
			echo GetHtmlSelect($z_id,$z_name,$arr_tamb,'ตำบล/แขวง',$val,'onchange="getZip(this.id,this.value,\''.$zip_id.'\')"','','1');
		}catch(Exception $e){
			$text=$e->getMessage();
		}
	break;
	
	case "zipcode" : //รหัสไปรษณีย์
		try{
	
			$v_zip=$_POST['v_zip'];
			$sql="SELECT top 1 TAMB_ZIPCODE FROM SETUP_TAMB WHERE  TAMB_ID='".$v_zip."' ";
			$query = $db->query($sql);
			$data = $db->db_fetch_array($query);
			
			echo $data['TAMB_ZIPCODE'];
		}catch(Exception $e){
			$text=$e->getMessage();
		}
	break;
	...
	case "edu_degree" : //วุฒิการศึกษา 
		try{	
			...
			$z_name=$_POST['z_name'];
			$z_class=$_POST['z_class'];
			 $sql = "SELECT EL_GROUP FROM SETUP_EDU_LEVEL WHERE EL_ID = '".$EL_ID."' ";
			 $query = $db->query($sql);
			 $rec = $db->db_fetch_array($query);
			 $sql_degree = "select ED_ID , ED_NAME_TH from SETUP_EDU_DEGREE WHERE ACTIVE_STATUS='1' AND DELETE_FLAG='0' AND EL_GROUP = '".$rec['EL_GROUP']."' ORDER BY ED_NAME_TH ASC "; 
			echo get_Select(
				$sql_degree ,
				$db ,
				array(
	...
	case "get_org_4" : {
		...
		$query = $db->query($sql);
		echo "<select id=\"S_ORG_ID_4\" name=\"S_ORG_ID_4\" class=\"selectbox form-control\" placeholder=\"-ทั้งหมด-\"> onchange=\"getORG5(this.value,\ORG_ID_5\);";
	...
	case "get_line_1" : {
			$arr=GetSqlSelectArray("LINE_ID", "LINE_NAME_TH", "SETUP_POS_LINE", " ACTIVE_STATUS = 0 AND LG_ID = '".$lg_id."' AND POSTYPE_ID = '".$POSTYPE_ID."'  ", "LINE_NAME_TH");
		echo GetHtmlSelect($z_id.$key_index,$z_name.$key_index,$arr,'ทั้งหมด','','','','1');
	}break;
	case "get_manage" : {
		if($type_id < 4){
	...
	case "pos_type" :    //ระดับตำแหน่ง/กลุ่มงาน
		try{
			...
			$json["selectbox"] = GetHtmlSelect($z_id.$key_index,$z_name.$key_index,$arr,'ทั้งหมด','','onchange="getPosLevel(this.value,\''.$POSTYPE_ID.'\',\''.$key_index.'\');getPosLine(this.value,\''.$POSTYPE_ID.'\',\''.$key_index.'\');getPosManage(this.value,\''.$POSTYPE_ID.'\',\''.$key_index.'\');"','','1');
			if($mode == "json"){
	...
	case "pos_level" :    //ระดับตำแหน่ง/กลุ่มงาน
		try{
			...
			$arr=GetSqlSelectArray("LEVEL_ID", "LEVEL_NAME_TH", "SETUP_POS_LEVEL", " TYPE_ID= '".$PARENT_ID."' AND ACTIVE_STATUS='1' and DELETE_FLAG='0' AND POSTYPE_ID= '".$POSTYPE_ID."' ", "LEVEL_SEQ");
			
			$json["key_index"] = $key_index;
			$json["selectbox"] = GetHtmlSelect($z_id.$key_index,$z_name.$key_index,$arr,'ทั้งหมด','','','','1','300');
			if($mode == "json"){
				echo json_encode($json);	
			}else{
				echo $json["selectbox"];	
	...
	case "pos_line" : //ตำแหน่งในสายงาน
		try{
			...
			$json["key_index"] = $key_index;
			$json["selectbox"] = GetHtmlSelect($z_id.$key_index,$z_name.$key_index,$arr,'ทั้งหมด','','','','1');
			if($mode == "json"){ 
				echo json_encode($json);	
			}else{
				echo $json["selectbox"];	
	...
	case 'pos_detail' :
		$sql = "	SELECT b.TYPE_ID, b.TYPE_NAME_TH, c.LINE_ID, c.LINE_NAME_TH, d.LEVEL_ID, d.LEVEL_NAME_TH, e.MANAGE_ID, e.MANAGE_NAME_TH
						FROM POSITION_FRAME a 
							LEFT JOIN SETUP_POS_TYPE b ON b.TYPE_ID = a.TYPE_ID 
							LEFT JOIN SETUP_POS_LINE c ON c.LINE_ID = a.LINE_ID 
							LEFT JOIN SETUP_POS_LEVEL d ON d.LEVEL_ID = a.LEVEL_ID 
							LEFT JOIN SETUP_POS_MANAGE e ON e.MANAGE_ID = a.MANAGE_ID 
						WHERE a.POS_ID = '".$POS_ID."' ";
		$rec = $db->get_data_rec($sql);
		
		echo json_encode(convert_text($rec));
		
	break;
	
	break;
		case "dec" :    //ชั้นเครื่องราชอิสริยาภรณ์
		try{
			$DEF_ID=$_POST['DEF_ID'];		
			$z_id=$_POST['z_id'];
			$z_name=$_POST['z_name'];
			...
			if(!empty($DEF_ID)){
				$Cond .= " AND DEF_ID = '".$DEF_ID."' ";
			}else{
				$Cond .= " AND (DEF_ID IS NULL OR DEF_ID = '0') ";
			}
			$arr_dec=GetSqlSelectArray("DEC_ID", "DEC_NAME_TH", "SETUP_DECORATION", "ACTIVE_STATUS='1' and DELETE_FLAG='0' ".$Cond, "DEC_NAME_TH");
			
			echo GetHtmlSelect($z_id, $z_name, $arr_dec,'ชั้นเครื่องราชอิสริยาภรณ์','','','','1');
	...
	case "get_level":
		$sql = "SELECT LEVEL_ID , LEVEL_NAME_TH FROM SETUP_POS_LEVEL WHERE TYPE_ID = '".$type_id."' and POSTYPE_ID = '".$postype_id."' ORDER BY LEVEL_SEQ";
		$query = $db->query($sql);
		$obj = array();
		while($rec = $db->db_fetch_array($query)){
			$row['level_id']= $rec['LEVEL_ID'];
			$row['level_name_th'] = text($rec['LEVEL_NAME_TH']);
			array_push($obj,$row);
		}
	...
	case "get_lg":
		$sql = "SELECT LG_ID ,LG_NAME_TH FROM SETUP_POS_LINE_GROUP WHERE TYPE_ID = '".$type_id."' and POSTYPE_ID = '".$postype_id."' ";
		$query = $db->query($sql);
		$obj = array();
		while($rec = $db->db_fetch_array($query)){
			$row['lg_id']= $rec['LG_ID'];
			$row['lg_name_th'] = text($rec['LG_NAME_TH']);
			array_push($obj,$row);
		}
		echo json_encode($obj);
		exit;
	...
	case "get_Manage":
		$sql = "SELECT MANAGE_ID ,MANAGE_NAME_TH FROM SETUP_POS_MANAGE WHERE  POSTYPE_ID = '".$postype_id."' "; 
		$query = $db->query($sql);
		$obj = array();
		while($rec = $db->db_fetch_array($query)){
			$row['manage_id']= $rec['MANAGE_ID'];
			$row['manage_name_th'] = text($rec['MANAGE_NAME_TH']);
			array_push($obj,$row);
		}
		echo json_encode($obj);
		exit;
	...
	case "get_Line":
		$sql = "SELECT LINE_ID ,LINE_NAME_TH FROM SETUP_POS_LINE WHERE LG_ID = '".$lg_id."' and POSTYPE_ID = '".$postype_id."' ";
		$query = $db->query($sql);
		$obj = array();
		while($rec = $db->db_fetch_array($query)){
			$row['line_id']= $rec['LINE_ID'];
			$row['line_name_th'] = text($rec['LINE_NAME_TH']);
			array_push($obj,$row);
		}
		echo json_encode($obj);
		exit;
	...
	case "get_org":
		$sql = "SELECT ORG_ID , ORG_NAME_TH FROM SETUP_ORG WHERE ORG_PARENT_ID = '".$parent_id."' ORDER BY ORG_SEQ";
		$query = $db->query($sql);
		$obj = array();
		while($rec = $db->db_fetch_array($query)){
			$row['org_id']= $rec['ORG_ID'];
			$row['org_name_th'] = text($rec['ORG_NAME_TH']);
			array_push($obj,$row);
		}
		echo json_encode($obj);
		exit;
