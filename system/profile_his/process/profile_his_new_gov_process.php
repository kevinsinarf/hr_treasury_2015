...
switch($proc){
	...
	/*		$V_FILE_PIC='NULL';
			if($FILE_PIC_NEW['name']!=''||$FILE_PIC_NEW['name']!=NULL){
				$V_FILE_PIC=getFilenameUplaod($FILE_PIC,$path_a,$FILE_PIC_NEW);
			}*/
	...
	case "get_org" : 
	    $ORG_ID = $_POST['ORG_ID'];
		$obj = array();
		$sql = "SELECT ORG_ID, ORG_NAME_TH FROM  SETUP_ORG WHERE ORG_PARENT_ID = '".$ORG_ID."' AND ACTIVE_STATUS = '1' AND DELETE_FLAG = '0' ORDER BY ORG_SEQ ASC ";
		$query = $db->query($sql);
