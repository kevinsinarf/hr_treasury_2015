...
switch($proc){
	...
	case "edu_degree" : //วุฒิการศึกษา 
			...
			$obj = array();
			 ...
			 $sql_degree = "SELECT ED_ID,
			 (CASE WHEN ED_NAME_TH IS NULL OR ED_NAME_TH = '' THEN ED_NAME_EN ELSE ED_NAME_TH END) AS ED_NAME_TH 
			  FROM SETUP_EDU_DEGREE
			 ...
			 $quey_degree = $db->query($sql_degree); 
			 while($rec_degree = $db->db_fetch_array($quey_degree)){
				 $row['ID'] = $rec_degree['ED_ID'];
				 $row['VALUE'] = text($rec_degree['ED_NAME_TH']);
				 array_push($obj,$row);
			 }
