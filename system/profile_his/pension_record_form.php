...
if($S_PENSION_IDCARD != ''){
	$query = $db->query("SELECT TOP 1  * FROM PENSION_MAIN A 
								...
								WHERE A.PENSION_IDCARD = '".str_replace('-','',$S_PENSION_IDCARD)."' AND A.DELETE_FLAG = 0 AND A.POSTYPE_ID = 1 ORDER BY PENSION_TIME DESC ");
	$num  = $db->db_num_rows($query);
	$query_all = $db->query("SELECT  * FROM PENSION_MAIN A 
	                             JOIN SETUP_POSITION_TYPE B ON A.POSTYPE_ID = B.POSTYPE_ID 
								WHERE A.PENSION_IDCARD = '".str_replace('-','',$S_PENSION_IDCARD)."' AND A.DELETE_FLAG = 0 AND A.POSTYPE_ID = 1 ORDER BY PENSION_TIME DESC ");
	$num_all = $db->db_num_rows($query_all);
	$PENSION_TIME = ($num_all+1);
	if($num > 0){
		$QueryMain = $query;
		...
		$PER_DATE_ENTRANCE = $rec_per['PER_DATE_ENTRANCE'];
		$PER_DATE_RETIRE = $rec_per['PER_DATE_RETIRE'];
		$PER_DATE_RESIGN = $rec_per['PER_DATE_RESIGN'];
		$PER_STATUS_PENALTY = $rec_per['PER_STATUS_PENALTY'];
		$PER_SALARY = $rec_per['PER_SALARY'];
		$SDATE_NOW = $rec_per['PER_DATE_ENTRANCE'];
		...
		$QueryMain = $db->query("SELECT * FROM  PER_PROFILE A JOIN SETUP_POSITION_TYPE B ON A.POSTYPE_ID = B.POSTYPE_ID  
											WHERE PER_IDCARD =  '".str_replace('-','',$S_PENSION_IDCARD)."' AND A.DELETE_FLAG = 0 AND ACTIVE_STATUS = 1 AND A.POSTYPE_ID = 1 ");
		$rec_main  = $db->db_fetch_array($QueryMain);	

		
	//PER_PROFILE 
	$name =  Showname($rec_main["PREFIX_ID"],$rec_main["PER_FIRSTNAME_TH"],$rec_main["PER_MIDNAME_TH"],$rec_main["PER_LASTNAME_TH"]);
	$POSTYPE_NAME = text($rec_main['POSTYPE_NAME_TH']);
	$PER_ID = $rec_main['PER_ID'];
	$SEX = $arr_gender[$rec_main['PER_GENDER']];
	$PER_DATE_ENTRANCE = $rec_main['PER_DATE_ENTRANCE'];
	$PER_DATE_RETIRE = $rec_main['PER_DATE_RETIRE'];
	$PER_DATE_RESIGN = $rec_main['PER_DATE_RESIGN'];
	$PER_STATUS_PENALTY = $rec_main['PER_STATUS_PENALTY'];
	$PER_SALARY = $rec_main['PER_SALARY'];
	$SDATE_NOW = $rec_main['PER_DATE_ENTRANCE'];
	if(trim($PER_DATE_RESIGN) != ''){
		$EDATE_NOW = $PER_DATE_RESIGN;  
	}else{
		$EDATE_NOW = $PER_DATE_RETIRE;  
...
<script src="js/pension_record_form.js?<?php echo rand(); ?>"></script>
</head>
       ...
       <input type="hidden" id="PER_ID" name="PER_ID"  value="<?php echo $PER_ID; ?>">
        ...
        <div class="panel-group" id="accordion"> 
            ...
            <div id="collapse1" class="collapse in">
             ...
             <?php } ?>
              ...
              <div class='row formSep'>
                  ...
                  <div class='col-xs-12 col-md-3'><?php echo $arr_per_status[$rec['PER_STATUS']]; ?></div>
              </div>
              <div class='row formSep'>
                 ...
                 <div class="col-xs-12 col-md-2" style="white-space:nowrap;">ครั้งที่เสนอขอรับ :&nbsp;<span style="color:red;">*</span></div>
               	 <div class="col-xs-12 col-md-1">
                    	<input type="text" id="PENSION_TIME" name="PENSION_TIME" class="form-control number" value="<?php echo $PENSION_TIME; ?>" readonly >
                    </div>
               </div>
                <div class="row formSep">
            ...
            <div id="collapse9" class="collapse">
                 <?php
				    $PENSION_SALARY = $PER_SALARY;
					$PENSION_ALL = $PENSION_SALARY * $total_year_cal;
				 ?>
            	 <div class="row formSep">
                <div class="col-xs-12 col-md-2" style="white-space:nowrap;">เงินเดือน เดือนสุดท้าย : </div>
                <div class="col-xs-12 col-md-2">
                    <input type="text" id="PENSION_SALARY" name="PENSION_SALARY" class="form-control" value="<?php echo number_format($PENSION_SALARY,2); ?>" style="display:inline-table; width:200px; margin-right:5px;" onBlur="NumberFormat(this,2);">
                </div>
                <div class="col-xs-12 col-md-1"></div>
                <div class="col-xs-12 col-md-2" style="white-space:nowrap;">เงินบำเหน็จที่ได้รับ :</div>
                <div class="col-xs-12 col-md-2">
                 	<input type="text" id="PENSION_ALL" name="PENSION_ALL" class="form-control " value="<?php echo number_format($PENSION_ALL,2); ?>" onBlur="NumberFormat(this,2);" >
                </div>
            </div>	
            </div>
            </div>
            ...
            <div id="collapse7" class="collapse">
                <?php
				 $MONTHLYNONGPG  = ($PER_SALARY/50)*$total_year_cal;     
				?>
            	<div class="row formSep">
                    <div class="col-xs-12 col-md-2" style="white-space:nowrap;">เงินเดือน เดือนสุดท้าย :&nbsp;</div>
                    <div class="col-xs-12 col-md-2">
						<input type="text" id="PENSION_SALARY" name="PENSION_SALARY" class="form-control" value="<?php echo number_format($PER_SALARY,2); ?>" onBlur="NumberFormat(this,2);" >
                    </div>
                    <div class="col-xs-12 col-md-1"></div>
                    <div class="col-xs-12 col-md-2" style="white-space:nowrap;">เงินบำนาญที่ได้รับต่อเดือน :</div>
                    <div class="col-xs-12 col-md-2">
                    	<input type="text" id="PENSION_MONTHLY" name="PENSION_MONTHLY" class="form-control " value="<?php echo number_format($MONTHLYNONGPG,2); ?>" onBlur="NumberFormat(this,2);">
                    </div>
                </div>
            ...
            <div id="collapse8" class="collapse">
                ...
                <table class="table table-bordered table-striped table-hover table-condensed">
					...
					while($num_month < 60){
						...
						$EDATE_SAL = '';
						$SALARY_PENSION = 0;
						$sql_salary = "SELECT A.SALHIS_ID, A.COM_SDATE, A.SALARY, 
						(SELECT TOP 1 B.COM_SDATE FROM PER_SALARYHIS B WHERE B.DELETE_FLAG = 0 AND B.ACTIVE_STATUS = 1  AND B.SALHIS_ID < A.SALHIS_ID AND A.PER_ID = B.PER_ID ORDER BY B.COM_SDATE DESC )AS SDATE,
						(SELECT TOP 1 B.SALARY FROM PER_SALARYHIS B WHERE B.DELETE_FLAG = 0 AND B.ACTIVE_STATUS = 1  AND B.SALHIS_ID < A.SALHIS_ID AND A.PER_ID = B.PER_ID ORDER BY B.COM_SDATE DESC )AS SALARY_1,
						(SELECT TOP 1 C.SALHIS_ID FROM PER_SALARYHIS C WHERE C.DELETE_FLAG = 0 AND C.ACTIVE_STATUS = 1  AND C.SALHIS_ID < A.SALHIS_ID AND A.PER_ID = C.PER_ID ORDER BY C.COM_SDATE DESC )AS SALHIS_ID_1
						FROM PER_SALARYHIS A WHERE A.DELETE_FLAG = 0 AND ACTIVE_STATUS = 1 AND PER_ID = '".$PER_ID."' AND 
						A.SALHIS_ID NOT IN(SELECT TOP ".$b." SALHIS_ID FROM PER_SALARYHIS WHERE DELETE_FLAG = 0 AND ACTIVE_STATUS = 1 AND PER_ID = '".$PER_ID."' ORDER BY COM_SDATE DESC )
						ORDER BY COM_SDATE DESC";
						$query_salary = $db->query($sql_salary);
						$num_salary = $db->db_num_rows($query_salary);
						if($num_salary == 0){
							break;
						}
						$rec_sal = $db->db_fetch_array($query_salary);
						
				       if($b == 0 and $c == 0 ){
							$SDATE_SAL = $rec_sal['COM_SDATE']; 
							$EDATE_SAL = date("Y-m-d", strtotime("-1 DAY", strtotime($EDATE_NOW)));
							$SALARY_PENSION = $rec_sal['SALARY'];
							$b = -1;
							$c = 1;
							$index_sal = $rec_sal['SALHIS_ID'];
					   }else if($b == 0 and $c== 1){
						   $c = 0;
						   $SDATE_SAL = $rec_sal['SDATE']; 
						   $EDATE_SAL = date("Y-m-d", strtotime("-1 DAY", strtotime($rec_sal['COM_SDATE'])));
						   $SALARY_PENSION = $rec_sal['SALARY_1'];
						   $index_sal = $rec_sal['SALHIS_ID_1'];
					   }else{
						   	$SDATE_SAL = $rec_sal['SDATE']; 
							$EDATE_SAL = date("Y-m-d", strtotime("-1 DAY", strtotime($rec_sal['COM_SDATE'])));
							$SALARY_PENSION = $rec_sal['SALARY_1'];
						...
						$query_num_sal = $db->query("SELECT DATEDIFF(month, '".$SDATE_SAL."', '".$EDATE_SAL."') AS NUM_MONTH ");
						$rec_num_sal = $db->db_fetch_array($query_num_sal);
						$month_salary = ($rec_num_sal['NUM_MONTH']+1);
					    $num_month = $num_month + $month_salary ;
						$num_money = $month_salary*$SALARY_PENSION;
				     ...
				     ?>
                      <tr bgcolor="#FFFFFF">
                          <td align="center"><?php echo $i; ?>.</td>
                          <td align="center">
                            <?php  echo conv_date($SDATE_SAL,'short');  ?>
                          	<input type="hidden" id="SALHIS_ID_<?php echo $i; ?>" name="SALHIS_ID[<?php echo $index_sal; ?>]" value="<?php echo $index_sal; ?>" >
                          	<input type="hidden"  name="UPS_SDATE[<?php echo $index_sal; ?>]" id="UPS_SDATE_<?php echo $i ?>" value="<?php  echo conv_date($SDATE_SAL);  ?>" >
                      
                          </td>
                          <td align="center">
                            <?php echo conv_date($EDATE_SAL,'short'); ?>
                          	<input type="hidden"  name="UPS_EDATE[<?php echo $index_sal; ?>]" id="UPS_EDATE_<?php echo $i; ?>" value="<?php echo conv_date($EDATE_SAL); ?>" >
                             
                          </td>
                          <td align="right">
                          	<?php echo number_format($SALARY_PENSION,2); ?>
                          	<input type="hidden" id="UPS_SALARY_<?php echo $i; ?>" name="UPS_SALARY[<?php echo $index_sal; ?>]" class="form-control " value="<?php echo $SALARY_PENSION; ?>" >
                          </td>
                          <td align="center">						  
                              <?php echo  $month_salary; ?>
                         	 <input type="hidden" id="UPS_MONTH_<?php echo $i; ?>" name="UPS_MONTH[<?php echo $index_sal; ?>]" class="form-control " value="<?php echo $month_salary; ?>" >
                          </td>
                          <td align="right">
                          <input type="hidden" name="UPS_TOTAL[<?php echo $index_sal; ?>]" id="UPS_TOTAL_<?php echo $i ?>" value=" <?php echo number_format($num_money,2);?>" >
