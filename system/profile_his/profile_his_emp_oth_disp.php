...
$pk_id=" a.PER_ID";
$wh=" a.DELETE_FLAG='0' AND a.POSTYPE_ID='".$POSTYPE_ID."' AND a.ACTIVE_STATUS = '1' {$filter}";
...
<script src="js/profile_his_emp_oth_disp.js?<?php echo rand(); ?>"></script>
</head>
  ...
  <div class="col-xs-12 col-sm-12" id="content">
    <div class="groupdata">
		...
		<div class="row">
			...
			<div class="col-xs-12 col-sm-3"><input type="text" id="s_idcard" name="s_idcard" class="form-control " placeholder="<?php echo $arr_txt['idcard'];?>" value="<?php echo $s_idcard; ?>"></div>
        ...
        <div class="row">
            ...
            <select id="s_org_3" name="s_org_3" class="selectbox form-control" placeholder="--ทั้งหมด--" onChange="getOrg(this.value, 's_org_4');">
				<option value=""></option>
            ...
            <table width="397" class="table table-bordered table-striped table-hover table-condensed">
              <thead>
                <tr class="bgHead">
                  ...
                  <th width="10%"><div align="center"><strong>จำนวนข้อมูล<br>รอนำเข้า</strong></div></th>
				...
				if($nums > 0){
					...
					while($rec = $db->db_fetch_array($query)){
						$PER_ID = $rec['PER_ID'];
						$sql_num_role = "SELECT PENALTY_ID, INFORM_NO, INFORM_DATE, INFORM_BY_PREFIX_ID, INFORM_BY_FIRSTNAME_TH, INFORM_BY_MIDNAME_TH, INFORM_BY_LASTNAME_TH, CRIME_NAME_TH, PENALTY_STATUS 
						FROM PENALTY_PETITION_FORM JOIN SETUP_CRIME_MAIN ON SETUP_CRIME_MAIN.CRIME_ID = PENALTY_PETITION_FORM.INFORM_CRIME_ID 
						WHERE INFORM_TO_PER_ID = '".$PER_ID."' AND TRANSFER_STATUS = '0'
						ORDER BY INFORM_DATE DESC";
						  $query_num_role = $db->query($sql_num_role);
						  $sql_num_bonus = "SELECT * FROM BONUS_COMMAND a INNER JOIN BONUS_ADJUST b ON a.BONUS_COM_ID = b.BONUS_COM_ID WHERE b.PER_ID = '".$PER_ID."'  ";
						  ...
						  WHERE B.ROTDESC_PER_ID = '".$PER_ID."' AND A.DELETE_FLAG = 0 AND B.TRANSFER_STATUS = 0
						  GROUP BY A.ROTCOM_ID, A.ROTCOM_NO, A.ROTCOM_DATE, A.ROTCOM_TITLE, A.ROTCOM_SDATE   ";
						  $query_num_app =  $db->query($sql_num_app);
						  $sql_num_salary = "SELECT A.SAL_COM_ID, A.YEAR_BDG, A.ROUND, A.COM_NO, A.COM_DATE, A.COM_TITLE, A.COM_SDATE  FROM SAL_COMMAND A 
						  INNER JOIN SAL_UP_SALARY B ON A.SAL_COM_ID = B.SAL_COM_ID 
						  WHERE B.PER_ID = '".$PER_ID."' AND A.DELETE_FLAG = 0 AND B.TRANSFER_STATUS = 0
						  GROUP BY A.SAL_COM_ID, A.YEAR_BDG, A.ROUND, A.COM_NO, A.COM_DATE, A.COM_TITLE, A.COM_SDATE   ";
						  ...
						  $total = $num_bonus+$num_role+$num_app+$num_salary;
						
						$transfer = "<a data-toggle=\"modal\" class=\"btn btn-default btn-xs\" data-backdrop=\"static\" href=\"javascript:void(0);\" onClick=\"transferData('".$rec["PER_ID"]."');\">".$img_edit." โอนข้อมูลเข้าทะเบียนประวัติ</a> ";
						?>
						<tr bgcolor="#FFFFFF">
