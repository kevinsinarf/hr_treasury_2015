$(document).ready(function(){
	...
	if($('#PENSION_TYPE_GPF').val() == ''){
	   $('#PENSION_TYPE_GPF option[value=1]').prop('selected',true);
	   $('#PENSION_TYPE_GPF').trigger('liszt:updated');
	}
	...
	if($('#PENSION_TYPE_REQUEST_CIVIL').val() == ''){
	   $('#PENSION_TYPE_REQUEST_CIVIL option[value=4]').prop('selected',true);
	   $('#PENSION_TYPE_REQUEST_CIVIL').trigger('liszt:updated');
	}
	...
	if($('#PENSION_TYPE_PENSION').val() != '' && $('#PENSION_TYPE_RESIGN').val() != '' && $('#PENSION_TYPE_GPF').val() != ''  ){
		GetContent();
	}
...
function GetPension(){
	...
	if(PENSION_TYPE_RESIGN == 6){
		...
		html += "<option value = '2'>บำนาญ</option>";
		html += "<option value = '3'>บำเหน็จตกทอด</option>"
	}
...
function GetContent(){
	var PENSION_TYPE_PENSION = $('#PENSION_TYPE_PENSION').val();
	var PENSION_TYPE_RESIGN = parseInt($('#PENSION_TYPE_RESIGN').val());
	var PENSION_TYPE_GPF = $('#PENSION_TYPE_GPF').val();
	var SALARYGPF = parseFloat($('#div_3  #PENSION_MONTHLY').val().split(',').join(''));
	var SALARYNONGPF = parseFloat($('#div_2 #PENSION_MONTHLY').val().split(',').join(''));
	var LEGENCY_ALL = 0;
	var BONUSTIME_ALL = 0;
	GetReceiverDetail();
	if(PENSION_TYPE_PENSION == 1){
		...
		$('#div_2 input').prop('disabled',true);
		$('#div_3 input').prop('disabled',true);
		$('#div_4 input').prop('disabled',true);
		
	}else{
		$('#div_4').show();
		$('#div_4 input').prop('disabled',false);
		if(PENSION_TYPE_GPF == 2){
			
			LEGENCY_ALL = (SALARYGPF*30); 
			BONUSTIME_ALL = (SALARYGPF*15); 
			if(BONUSTIME_ALL >= 400000){
				BONUSTIME_ALL = 400000;
			}
			$('#div_4 #LEGENCY_ALL').val(number_format_return(LEGENCY_ALL,2));
			$('#div_4 #BONUSTIME_ALL').val(number_format_return(BONUSTIME_ALL,2));
			ChkMoneyBonus($('#div_4 #BONUSTIME_AMOUNT'));
			
			$('#div_2').show();
			$('#div_3').hide();
			$('#div_2 input').prop('disabled',false);
			$('#div_3 input').prop('disabled',true);
			
		}else{
			
			LEGENCY_ALL = (SALARYNONGPF*30); 
			BONUSTIME_ALL = (SALARYNONGPF*15); 
			if(BONUSTIME_ALL >= 400000){
				BONUSTIME_ALL = 400000;
			}
			$('#div_4 #LEGENCY_ALL').val(number_format_return(LEGENCY_ALL,2));
			...
			$('#div_3').show();
			$('#div_2').hide();
			$('#div_3 input').prop('disabled',false);
			$('#div_2 input').prop('disabled',true);
		}
	}
...
function chkinput(){	
	...
	if($('#PENSION_TYPE_GPF').val() == ''){
		alert('กรุณาเลือก สถานะการเป็นสมาชิก กบข');
		$('#PENSION_TYPE_GPF').focus();
		return false;
	}
	if($('#PENSION_TYPE_RESIGN').val() == ''){
...
function getRampr(obj){	
	...
	var html = "<option value=''></option>";
	$.ajax({
		url: "process/pension_record_process.php",
		dataType: "json",   
		type: "POST",
...
function getZipcode(obj){
	var TAMB_ID = $(obj).val();
	$.post('process/pension_record_process.php',{proc:'getZipcode','TAMB_ID':TAMB_ID},function(data){
		$('#ADDRESS_ZIPCODE').val(data);
    });
}
function getBonustime(){
	var BONUSTIME_RECEIVE =  $('input:radio[name^=BONUSTIME_RECEIVE]:checked').val();
	if(BONUSTIME_RECEIVE == 1 ){
		$('#BONUSTIME_AMOUNT').prop('disabled',false);
	}else{
		$('#BONUSTIME_AMOUNT').prop('disabled',true);
		var LEGENCY_ALL = parseFloat($('#LEGENCY_ALL').val().split(',').join(''));
		var BONUSTIME_ALL = parseFloat($('#BONUSTIME_ALL').val().split(',').join(''));
		$('#LEGENCY_BALANCE').val(number_format_return(LEGENCY_ALL,2));
		$('#BONUSTIME_BALANCE').val(number_format_return(BONUSTIME_ALL,2));
		$('#BONUSTIME_AMOUNT').val('');
	}
}
function ChkMoneyBonus(obj){
	var MONEY = parseFloat($(obj).val().split(',').join(''));
	var LEGENCY_ALL = parseFloat($('#LEGENCY_ALL').val().split(',').join(''));
	var BONUSTIME_ALL = parseFloat($('#BONUSTIME_ALL').val().split(',').join(''));
	var TOTAL = LEGENCY_ALL - MONEY;
	var TOTAL_BONUSTIME = BONUSTIME_ALL - MONEY;
	$('#LEGENCY_BALANCE').val(number_format_return(TOTAL,2));
	$('#BONUSTIME_BALANCE').val(number_format_return(TOTAL_BONUSTIME,2));
}
var isMobile = {
	Android: function() {
