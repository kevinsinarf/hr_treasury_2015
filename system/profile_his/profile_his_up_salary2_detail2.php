...
if($data['MOVEMENT_ID']==15||$data['MOVEMENT_ID']==22||$data['MOVEMENT_ID']==26){
		$filter2 = "  A.SAL_COM_ID = ".$SAL_COM_ID; 
		$text = "เงินเดือน";
		
}
if($data['MOVEMENT_ID']==17||$data['MOVEMENT_ID']==23||$data['MOVEMENT_ID']==45){
		$filter2 = " A.SAL_COM_TEMP = ".$SAL_COM_ID; 
		$text = "เงินค่าครองชีพชั่วคราว";
}

if($data['MOVEMENT_ID']==16||$data['MOVEMENT_ID']==44||$data['MOVEMENT_ID']==27){
		$filter2 = " A.SAL_COM_SPE =  ".$SAL_COM_ID; 
		$text = "เงินค่าตอบแทนพิเศษ";
}


$sql = "SELECT A.YEAR_BDG, A.ROUND, B.COM_TITLE, B.COM_NO, B.COM_SDATE, B.COM_DATE, C.PREFIX_ID, C.PER_FIRSTNAME_TH, C.PER_MIDNAME_TH, C.PER_LASTNAME_TH, D.ORG_NAME_TH, A.SALARY_NEW, E.LEVEL_NAME_TH, G.LINE_NAME_TH ,A.SALARY_SPE_UP,A.SAL_COMPENSATION_4
					...
					<div class="table-responsive">
                        <table width="397" class="table table-bordered table-striped table-hover table-condensed">
                          <thead>
                              ...
                              <th width="10%"  style="text-align:center; vertical-align:middle" ><strong><?php echo $text;?></strong></th>
                           </tr>
                           </thead>
							<tbody>
							...
							if ($nums > 0) {
								...
								while($rec= $db->db_fetch_array($query)) {
								 	...
								 	if($data['MOVEMENT_ID']==15||$data['MOVEMENT_ID']==22||$data['MOVEMENT_ID']==26){
											...
											$money =($rec['SALARY_NEW']);
									}
									if($data['MOVEMENT_ID']==17||$data['MOVEMENT_ID']==23||$data['MOVEMENT_ID']==45){
											 
											$money =($rec['SAL_COMPENSATION_4']);
									}
							
									if($data['MOVEMENT_ID']==16||$data['MOVEMENT_ID']==44||$data['MOVEMENT_ID']==27){
										
											$money =($rec['SALARY_SPE_UP']);
									}
									$total +=$money;// A.SALARY_SPE_UP,A.SAL_COMPENSATION_4
								 ?>
                           <tr bgcolor="#FFFFFF">
