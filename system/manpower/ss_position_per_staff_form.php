...
if($proc == 'add'){
	$query_frame  = $db->query("SELECT MAX(CAST(POS_NO AS INT)) AS POS_NO_MAX FROM POSITION_FRAME WHERE POSTYPE_ID = 5 AND ORG_ID_2 = 15 ");
	...
	LEFT JOIN SETUP_POS_LEVEL B ON A.LEVEL_ID = B.LEVEL_ID
	 WHERE A.POS_ID = '".$data['POS_ID']."' AND A.POSTYPE_ID = 5 ";
...
//$array
$arr_level=GetSqlSelectArray("LEVEL_ID", "LEVEL_NAME_TH", "SETUP_POS_LEVEL", "ACTIVE_STATUS='1' AND POSTYPE_ID = 5 and DELETE_FLAG='0' AND TYPE_ID = '".$data['TYPE_ID']."' ", "LEVEL_SEQ");
...
<script src="js/ss_position_per_staff_form.js?<?php echo rand(); ?>"></script>
</head>
<body>
<div class="container-full">
	...
	<div class="col-xs-12 col-md-12">
		<ol class="breadcrumb">
			...
			<li><a href="ss_position_per_staff_disp.php?<?php echo $paramlink; ?>"><?php echo showMenu($menu_sub_id); ?></a></li>
			<li class="active"><?php echo $txt; ?></li>
		</ol>
	</div>
	<div class="col-xs-12 col-sm-12" id="content">
		<div class="groupdata" >
			<form id="frm-input" method="post" action="process/ss_position_per_staff_process.php"enctype="multipart/form-data">
			  ...
			  <div class="row formSep">
                ...
                <div class="col-xs-12 col-md-2 col-md-offset-1" style="white-space:nowrap;">กลุ่ม : <span style="color:red;">*</span> </div>
					<div class="col-xs-12 col-md-3">
                    	<select id="TYPE_ID" name="TYPE_ID" class="selectbox form-control" placeholder="กลุ่ม" onChange="get_level(this); get_line_group(this);">
							<option value=""></option>
							<?php
					...
					<div class="col-xs-12 col-md-3">
						<select id="LINE_ID" name="LINE_ID" class="selectbox form-control" placeholder="ตำแหน่งในสายงาน" >
							...
							if(!empty($data['LINE_ID'])){
								$sql_line_name = "Select LINE_ID , LINE_NAME_TH From SETUP_POS_LINE WHERE ACTIVE_STATUS = 1 AND LG_ID = '".$data['LG_ID']."' AND POSTYPE_ID = 5 ORDER BY LINE_NAME_TH ASC";
                ...
                <div class="row formSep">
					...
					<div class="col-xs-12 col-md-3">
						<?php echo GetHtmlSelect('LEVEL_ID', 'LEVEL_ID',$arr_level , 'ระดับตำแหน่ง' ,$data['LEVEL_ID'] ,'', '1', '', ''); ?>
					</div>
                    <div class="col-xs-12 col-md-1"></div>
                    <div class="col-xs-12 col-md-2 " style="white-space:nowrap;">หมายเหตุของตำแหน่ง :</div>
             		<div class="col-xs-12 col-md-3">
						<?php echo GetHtmlSelect('REMARK_ID', 'REMARK_ID',$arr_remark, 'หมายเหตุของตำแหน่ง' ,$data['REMARK_ID'] ,'', '1', '', ''); ?>		
					</div>
                </div>
                <div class="row formSep">
               ...
               <?php if($proc == 'edit'){ ?>
				...
				<div class="row formSep">
					<div class="col-xs-4 col-md-2" style="white-space:nowrap;">ค่าจ้าง :</div>
					<div class="col-xs-4 col-md-3" >
				...
				<div class="row head-form">ข้อมูลค่าจ้าง</div>
					
				<div class="row formSep">
					<div class="col-xs-4 col-md-2" style="white-space:nowrap;">ค่าจ้าง : <span style="color:red;">*</span></div>
                ...
                <div class="row formSep">
                	<div class="col-xs-12 col-md-2" style="white-space:nowrap;">แหล่งของเงิน : </div>
                    <div class="col-xs-12 col-md-3">
                    	<select id="MAN_BDG_ID" name="MAN_BDG_ID" class="selectbox form-control" placeholder="แหล่งของเงิน">
                        	<option value=""></option>
                            <?php
								$sql_bdg_mon = "SELECT MAN_BDG_ID,MAN_BDG_NAME_TH FROM SETUP_MANPOWER_BUDGET ORDER BY MAN_BDG_ID DESC";
								$query_bdg_mon = $db->query($sql_bdg_mon);
						 		while($rec_bdg_mon = $db->db_fetch_array($query_bdg_mon)){
									?>
                                    <option value="<?php echo $rec_bdg_mon['MAN_BDG_ID'];?>" <?php echo  ($data['MAN_BDG_ID']==$rec_bdg_mon['MAN_BDG_ID'])?"selected":"";  ?> ><?php echo text($rec_bdg_mon['MAN_BDG_NAME_TH']);?></option>
									<?php 
								}
							?>
                        </select>
                    ...
                    <div class="col-xs-12 col-md-1"></div>
                    <div class="col-xs-12 col-md-2"  style="white-space:nowrap;">วันที่ยกเลิกตำแหน่ง :</div>
					<div class="col-xs-12 col-md-2">
						<div id="CANCEL_POS_DATE_1" class="input-group datepicker">
							<input type="text" id="CANCEL_POS_DATE" name="CANCEL_POS_DATE" class="form-control" placeholder="DD/MM/YYYY" maxlength="10" value="<?php echo conv_date(text($data['CANCEL_POS_DATE'])); ?>">
							<span class="input-group-addon datepicker" for="CANCEL_POS_DATE">&nbsp;<span class="glyphicon glyphicon-calendar"></span>&nbsp;</span>
						</div>
					</div>
                </div>
				<div class="formlast">
					...
					<button type="button" class="btn btn-default" onClick="self.location.href='ss_position_per_staff_disp.php?<?php echo url2code("menu_id=".$menu_id."&menu_sub_id=".$menu_sub_id);?>';">ยกเลิก</button>
					</div>
				</div>
			</form>
		</div>
