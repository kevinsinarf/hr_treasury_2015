...
$org_prov_id = $rec["org_prov_id"];
$org_ampr_id = $rec["org_ampr_id"];
$org_tamb_id = $rec["org_tamb_id"];
$org_tel = $rec["org_tel"];
$org_fax = $rec["org_fax"];
...
<script src="js/disp_senate.js?<?php echo rand(); ?>"></script>
</head>
<body>
    ...
    <div>
        <div class="col-xs-12 col-sm-12">
            <ol class="breadcrumb">
              ...
              <li><a href="disp_senate.php?<?php echo $paramlink; ?>">สำนักงานเลขาธิการวุฒิสภา</a></li>
              <li class="active"><?php echo $txt; ?></li>
            </ol>
        </div>
        <div class="col-xs-12 col-sm-12" id="content">
            <div class="groupdata" >
                <form id="frm" method="post" action="process/process_senate.php">
                    ...
                    <div class="col-xs-12 col-sm-3"><label><span id="org_year2" name="org_year2"><?php echo $org_year; ?></span></label>
                                                    <input name="org_year" class="form-control number hidden" id="org_year" type="text" value="<?php echo $org_year; ?>"></div>
                    ...
                    <div class="col-xs-12 col-sm-3"><?php while($recType = $db->db_fetch_array($Qtype)){
                                                    $sel = ($ot_id == $recType["ot_id"]) ? "$ot_id":"";?>
                        <label><span id="ot_id2" name="ot_id2"><?php echo text($recType["ot_name_th"]); ?></span></label>
                        <input name="ot_id" id="ot_id" type="text" value="<?php echo $sel;?>" hidden>
                    <?php }?>
                    </div>
                    <div class="col-xs-12 col-sm-1"></div>
                    ...
                    <div class="col-xs-12 col-sm-3"><?php while($recOL = $db->db_fetch_array($QOL)){ 
                        ...
                        <input id="ol_id" name="ol_id" class="form-control hidden" placeholder="ปีที่" value="<?php echo $recOL["ol_id"]; ?>">
                    <?php }?>
                    </div>
                    </div>
                    <div class="row formSep">
                    ...
                    <div class="col-xs-12 col-sm-3"><label><span id="org_name_en2" name="org_name_en2"><?php echo $org_name_en; ?></span></label>
                    ...
                    <div class="row formSep">
         			<?php 
					$sql_ssp_type="SELECT SSA_ID,SS_ID,SSA_TAMB_ID,SSA_AMPR_ID,SSA_PROV_ID,ACTIVE_STATUS,DELETE_FLAG
					FROM SS_ADDRESS WHERE SS_ID='".$SS_ID."' AND ACTIVE_STATUS='1' AND DELETE_FLAG='0' ";
					$query_ssp_type = $db->query($sql_ssp_type);
					$rec1 = $db->db_fetch_array($query_ssp_type);
					//จังหวัด
					$arr_prov=GetSqlSelectArray("PROV_ID", "PROV_TH_NAME", "SETUP_PROV", "ACTIVE_STATUS='1' and DELETE_FLAG='0'", "PROV_TH_NAME");
					//อำเภอ/เขต
					$arr_ampr=GetSqlSelectArray("AMPR_ID", "AMPR_NAME_TH", "SETUP_AMPR", "PROV_ID='".$rec['org_prov_id']."' and ACTIVE_STATUS='1' and DELETE_FLAG='0'", "AMPR_NAME_TH");
					//ตำบล/แขวง
					$arr_tamb=GetSqlSelectArray("TAMB_ID", "TAMB_NAME_TH", "SETUP_TAMB", "AMPR_ID='".$rec['org_ampr_id']."' and ACTIVE_STATUS='1' and DELETE_FLAG='0'", "TAMB_NAME_TH");

					?>
       				<div class="col-xs-12 col-md-2" style="white-space:nowrap;">จังหวัด :</div>
                        <div class="col-xs-12 col-md-2"><?php echo GetHtmlSelect('prov_id','prov_id',$arr_prov,'จังหวัด',$rec['org_prov_id'],'onchange="getRampr(this,\'ampr_id\',\'tamb_id\')"','','1');?></div>
                        <div class="col-xs-12 col-md-1" style="white-space:nowrap;">อำเภอ/เขต :</div>
                        <div class="col-xs-12 col-md-2"><span id='ss_ampr'><?php echo GetHtmlSelect('ampr_id','ampr_id',$arr_ampr,'อำเภอ/เขต',$rec['org_ampr_id'],'onchange="getStamb(this.id,this.value,\'tamb_id\') " ','','1');?></span></div>
                        <div class="col-xs-12 col-md-1" style="white-space:nowrap;">ตำบล/แขวง :</div>
                        <div class="col-xs-12 col-md-2"><span id='ss_tamb'><?php echo GetHtmlSelect('tamb_id','tamb_id',$arr_tamb,'ตำบล/แขวง',$rec['org_tamb_id'],'','','1');?></span></div>            
					</div>
                    <div class="row formSep">
                    ...
                    <div class="col-xs-12 col-sm-12" align="center">
                        ...
                        <button type="button" class="btn btn-default" onClick="self.location.href='disp_senate.php?<?php echo url2code("menu_id=".$menu_id."&menu_sub_id=".$menu_sub_id);?>';">ยกเลิก</button>
                    </div>
                    </div> 
                </form>
            </div>
