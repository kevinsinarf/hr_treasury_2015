...
function addData(){
	$("#proc").val("add");
	$("#frm-search").attr("action","competency_management_year_form.php").submit();
}
...
function chkinput(){
	...
	if($("#MANAGE_ID").val() == ""){
		alert("ระบุ "+$('#MANAGE_ID').attr('placeholder'));
		$("#MANAGE_ID").focus();
		return false;
	}
...
function delData(id){
	if(confirm("ต้องการลบข้อมูลใช่หรือไม่ ?")){
		$("#proc").val("delete");
		$("#COMSET_ID").val(id);
		$("#frm-search").attr("action","process/competency_management_year_process.php").submit();
	}
}
function Chkrepeat(){
