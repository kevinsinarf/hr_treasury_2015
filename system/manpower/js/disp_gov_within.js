...
function addData(id,seq){
	$("#proc").val("add");
        $("#seq").val(seq);
        $("#org_id1").val(id); 
	$("#frm-search").attr("target","").attr("action","form_gov_within.php").submit();
}
...
function editData1(id){
	$("#proc").val("edit");
	$("#org_id").val(id);
	$("#frm-search").attr("target","").attr("action","form_gov_within_address.php").submit();
}
...
function deleteData(id){
	if(confirm("ต้องการลบข้อมูลหรือไม่ ?")){
		$("#proc").val("delete");
		$("#org_id").val(id);
		$("#frm-search").attr("target","").attr("action","process/process_gov_within.php").submit();
	}
}
