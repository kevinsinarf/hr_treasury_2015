...
if($proc == 'add'){
	$query_frame  = $db->query("SELECT MAX(CAST(POS_NO AS INT)) AS POS_NO_MAX FROM POSITION_FRAME WHERE POSTYPE_ID = 3 AND ORG_ID_2 = 15 ");
...
<script src="js/ss_position_per_employees_form.js?<?php echo rand(); ?>"></script>
</head>
<body>
<div class="container-full">
	...
	<div class="col-xs-12 col-md-12">
		<ol class="breadcrumb">
			...
			<li><a href="ss_position_per_employees_disp.php?<?php echo $paramlink; ?>"><?php echo showMenu($menu_sub_id); ?></a></li>
			<li class="active"><?php echo $txt; ?></li>
		</ol>
	</div>
	<div class="col-xs-12 col-sm-12" id="content">
		<div class="groupdata" >
			<form id="frm-input" method="post" action="process/ss_position_per_employees_process.php" enctype="multipart/form-data">
			  ...
			  <div class="row formSep">
					...
					<div class="col-xs-12 col-md-3" >
						<select id="LINE_ID" name="LINE_ID" class="selectbox form-control" placeholder="ตำแหน่ง" >
							...
							if(!empty($data['LINE_ID'])){
								$sql_line_name = "Select LINE_ID , LINE_NAME_TH From SETUP_POS_LINE WHERE ACTIVE_STATUS = 1 AND DELETE_FLAG = '0' AND LEVEL_ID = ".$data['LEVEL_ID']." AND POSTYPE_ID = 3  ORDER BY LINE_NAME_TH ASC";
                ...
                <input type= "file" id="POS_FILE" name="POS_FILE" class="form-control"  >
					<?php echo displayDownloadFileAttach($path_img,$data['POS_FILE'],$arr_txt['download']);?>
                </div>  
				...
				<div class="formlast">
					...
					<button type="button" class="btn btn-default" onClick="self.location.href='ss_position_per_employees_disp.php?<?php echo url2code("menu_id=".$menu_id."&menu_sub_id=".$menu_sub_id);?>';">ยกเลิก</button>
					</div>
				</div>
			</form>
		</div>
