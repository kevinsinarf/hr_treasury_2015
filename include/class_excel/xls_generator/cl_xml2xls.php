...
class xls_wrap { // for wrapping purposes
	...
	var $_cur_row;
	var $_cur_col;
	var $_cur_cell_index=false;
	//------------------------
	var $_styles;
	var $_cur_style=false;
	var $_cur_elt_style=array();
...
// ------------------------------------------------------------------------------------------------
	function &_get_style() {
		...
		while ($this->_tree_element[$lvl] != "WORKBOOK") {
			...
			if ($this->_cur_elt_style[$cur_element]) {
				return $this->_styles[$this->_cur_elt_style[$cur_element]];
			}
			if ($this->_cur_elt_format[$cur_element]) {
...
// ------------------------------------------------------------------------------------------------
	function _int_WORKBOOK($attribs,$EndOfTag=false) {
		if (!$EndOfTag) {
			$this->workbook =& new xls_book($this->FileName,$this->config_file,$this->temp_dir);
