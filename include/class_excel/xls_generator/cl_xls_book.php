...
// USER - adding new sheet to the given book
	function &add_sheet($name="") {
		...
		$worksheet =& new xls_sheet(	$this->_config_file,
												$name, 
												$index, 
												$this->_activesheet,
												$this->_firstsheet,
...
// ------------------------------------------------------------------------------------------------
	function &add_format($para=false) {
		if($para===false) {
			$format =& new xls_format($this->_config_file,$this->_xf_index);
		} else {
			$format =& new xls_format($this->_config_file,$this->_xf_index, $para);
		}
		$this->_xf_index += 1;
