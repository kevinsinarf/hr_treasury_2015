...
class xls_sheet extends xls_generator {
		...
		var $_dim_col_start;
		var $_title_row_end;
		var $_title_row_start;
		var $_title_col_end;
		var $_title_col_start;
		var $_print_row_end;
		var $_print_row_start;
		var $_print_col_end;
		var $_print_col_start;
...
// CONSTRUCTOR - creates new sheet in a given workbook
	function xls_sheet($c_file, $name, $index, &$activesheet, &$firstsheet, &$url_format, $tempdir) {
		if ($c_file) {
			include $c_file;
		}
		...
		$this->_dim_row_end			= $rowmax +1;
		$this->_dim_row_start		= 0;
		$this->_dim_col_end			= $colmax +1;
...
// _dump_hlink($Start_row, $Start_col, $End_row, $End_col, $url, $string, $format)
	function _dump_hlink() {
		...
		$xf = $Args[6] ? $Args[6] : $this->_url_format;
...
// _dump_selection($FirstRow, $FirstCol, $LastRow, $LastCol)
	function _dump_selection($Args) {
		...
		*/
		...
		$LastRowIndex = $Args[2] ? $Args[2] : $FirstRowIndex;
		$LastColIndex = $Args[3] ? $Args[3] : $FirstColIndex;
