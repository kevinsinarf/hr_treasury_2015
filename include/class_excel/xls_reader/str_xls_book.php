	...
	function get_html() {
		...
		for ($r=0;$r<=$this->num_rows;$r++) {
			...
			for ($c=0;$c<=$this->num_cols;$c++) {
				...
				if (sizeof($this->_cells_merge_info[$r][$c])>0) {
					//$data .= "mrg";
					$span = "";
...
// ------------------------------------------------------------------------------------------------
	function _convert_date($date,$format) {
		...
		if ($date_ar['day']<10) {"0".$date_ar['day'];}
		$l = (int) ($j / 11);
		// Fix read month wrong
		// Original: $date_ar['month'] = $j + 1 - (12 * $l);
		$date_ar['month'] = ($j + 1 - (12 * $l))+1;
		...
		$rpls = array (
					$date_ar['am_pm'],
					// Fix read month wrong
					// Original: $this->_parent->month_names[(int)$date_ar['month']],
					// Original: $this->_parent->month_names_short[(int)$date_ar['month']]
